...
namespace {
    ...
    return [
        ...
        'view.clearcompiledtemplates.enable' => \true,
                ...
                if (\filter_var($ip, \FILTER_VALIDATE_IP) || \Matomo\Network\IPUtils::getIPRangeBounds($ip) !== null) {
                    ...
                    if (!\is_array($resolvedIps)) {
                        ...
                        if (\function_exists('dns_get_record')) {
                            ...
                            if (!empty($entry['0']['ipv6']) && \filter_var($entry['0']['ipv6'], \FILTER_VALIDATE_IP)) {
         ...
         */
        'http.blocklist.hosts' => ['*.amazonaws.com'],
        'Piwik\\Tracker\\VisitorRecognizer' => \Piwik\DI::autowire()->constructorParameter('trustCookiesOnly', \Piwik\DI::get('ini.Tracker.trust_visitors_cookies'))->constructorParameter('visitStandardLength', \Piwik\DI::get('ini.Tracker.visit_standard_length'))->constructorParameter('lookbackNSecondsCustom', \Piwik\DI::get('ini.Tracker.window_look_back_for_visitor')),
        'Piwik\\Tracker\\Settings' => \Piwik\DI::autowire()->constructorParameter('isSameFingerprintsAcrossWebsites', \Piwik\DI::get('ini.Tracker.enable_fingerprinting_across_websites')),
        'archiving.performance.logger' => null,
        \Piwik\CronArchive\Performance\Logger::class => \Piwik\DI::autowire()->constructorParameter('logger', \Piwik\DI::get('archiving.performance.logger')),
