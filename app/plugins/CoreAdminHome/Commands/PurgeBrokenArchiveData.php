...
class PurgeBrokenArchiveData extends ConsoleCommand
{
    ...
    protected function configure()
    {
        ...
        $this->addOptionalArgument("startMonth", "The start month to purge data from in format YYYY-MM. Defaults to current month", $currentMonth->getDateStart()->toString('Y-m'));
        $this->addOptionalArgument("endMonth", "The end month to purge data to in format YYYY-MM. Defaults to current month", $currentMonth->getDateStart()->toString('Y-m'));
        $this->setHelp("Broken archives are removed from all archive tables between supplied months inclusive.\n\n" . "Note: archive purging is done during scheduled task execution, so under normal circumstances, you should not need to " . "run this command manually.");
    }
