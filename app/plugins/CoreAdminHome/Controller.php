...
class Controller extends ControllerAdmin
{
    ...
    public function __construct(Translator $translator, \Piwik\Plugins\CoreAdminHome\OptOutManager $optOutManager, PasswordVerifier $passwordVerify)
    {
    ...
    public function home()
    {
        ...
        return $this->renderTemplate('home', array('isInternetEnabled' => $isInternetEnabled, 'isMarketplaceEnabled' => $isMarketplaceEnabled, 'hasPremiumFeatures' => $hasPremiumFeatures, 'hasNewPlugins' => $hasNewPlugins, 'isFeedbackEnabled' => $isFeedbackEnabled, 'hasDonateForm' => $hasDonateForm, 'hasPiwikBlog' => $hasPiwikBlog, 'hasDiagnostics' => $hasDiagnostics, 'hasTrackingFailures' => $hasTrackingFailures, 'hasQuickLinks' => $hasQuickLinks, 'hasSystemSummary' => $hasSystemSummary));
    }
    ...
    public function generalSettings()
    {
        ...
        $logo = new \Piwik\Plugins\CoreAdminHome\CustomLogo();
        ...
        $view->hasUserLogo = \Piwik\Plugins\CoreAdminHome\CustomLogo::hasUserLogo();
        $view->pathUserLogo = \Piwik\Plugins\CoreAdminHome\CustomLogo::getPathUserLogo();
        $view->hasUserFavicon = \Piwik\Plugins\CoreAdminHome\CustomLogo::hasUserFavicon();
        $view->pathUserFavicon = \Piwik\Plugins\CoreAdminHome\CustomLogo::getPathUserFavicon();
        $view->pathUserLogoSmall = \Piwik\Plugins\CoreAdminHome\CustomLogo::getPathUserLogoSmall();
        $view->pathUserLogoSVG = \Piwik\Plugins\CoreAdminHome\CustomLogo::getPathUserSvgLogo();
        $view->pathUserLogoDirectory = realpath(dirname($view->pathUserLogo) . '/');
        $view->mailTypes = array('' => '', 'Plain' => 'Plain', 'Login' => 'Login', 'Cram-md5' => 'Cram-md5');
        $view->mailEncryptions = array('' => 'auto', 'ssl' => 'SSL', 'tls' => 'TLS', 'none' => 'none');
        $mail = new Mail();
    ...
    public function setMailSettings()
    {
        ...
        try {
            ...
            if (Piwik::doesUserRequirePasswordConfirmation($login)) {
                ...
                if ($passwordConfirmation === '' || !$this->passwordVerify->isPasswordCorrect($login, $passwordConfirmation)) {
    ...
    public function trackingCodeGenerator()
    {
        ...
        $view->defaultSiteDecoded = ['id' => $view->idSite, 'name' => Common::unsanitizeInputValue($view->defaultReportSiteName)];
    ...
    private function enrichChangesForWhatIsNew(array $changes) : array
    {
        ...
        foreach ($changes as &$change) {
            ...
            $change['showPluginPrefix'] = !empty($pluginName) && !$pluginManager->isPluginBundledWithCore($pluginName);
