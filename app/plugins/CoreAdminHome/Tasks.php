...
class Tasks extends \Piwik\Plugin\Tasks
{
    ...
    public function checkSiteHasTrackedVisits($idSite)
    {
        ...
        $email = $container->make(JsTrackingCodeMissingEmail::class, array('login' => $user['login'], 'emailAddress' => $user['email'], 'idSite' => $idSite));
        $email->send();
    }
    ...
    public function purgeBrokenArchivesCurrentMonth() : bool
    {
        ...
        if (!empty($archiveTable)) {
            $this->logger->info("Purging broken archives in {table} archive table.", ['table' => $archiveTable]);
    ...
    public function updateSpammerList()
    {
        ...
        if (preg_match('/[<>&?"\']/', $list)) {
            throw new \Exception(sprintf('The spammers list downloaded from %s contains unexpected characters, considering it a fail', $url));
        }
        $list = preg_split("/\r\n|\n|\r/", $list);
        if (count($list) < 10) {
