...
class Menu extends \Piwik\Plugin\Menu
{
    ...
    {
        if (Piwik::isUserHasSomeAdminAccess() && \Piwik\Plugins\UsersManager\UsersManager::isUsersAdminEnabled()) {
            $menu->addSystemItem('UsersManager_MenuUsers', $this->urlForAction('index'), $order = 15);
        }
        if (Piwik::hasUserSuperUserAccess() && \Piwik\Plugins\UsersManager\API::getInstance()->getSitesAccessFromUser('anonymous')) {
