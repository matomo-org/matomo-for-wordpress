...
class UserNotifierTask extends Task
{
    ...
    {
        return PluginManager::getInstance()->findMultipleComponents('UserNotifications', \Piwik\Plugins\UsersManager\UserNotifications\UserNotificationProviderInterface::class);
    }
    /**
    ...
    public function dispatchNotifications()
    {
        ...
        try {
            ...
            if ($notificationsToDispatchCount) {
                $logger->info("Number of user notifications dispatched: {number} of {total}.", ['number' => $notificationsDispatchedCount, 'total' => $notificationsToDispatchCount]);
            } else {
