...
class TokenNotifierTask extends Task
{
    ...
    {
        return PluginManager::getInstance()->findMultipleComponents('TokenNotifications', \Piwik\Plugins\UsersManager\TokenNotifications\TokenNotificationProviderInterface::class);
    }
    /**
    ...
    public function dispatchNotifications()
    {
        ...
        try {
            ...
            if ($notificationsToDispatchCount) {
                $logger->info("Number of token notifications dispatched: {number} of {total}.", ['number' => $notificationsDispatchedCount, 'total' => $notificationsToDispatchCount]);
            } else {
