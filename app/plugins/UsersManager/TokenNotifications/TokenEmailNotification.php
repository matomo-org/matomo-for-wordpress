...
abstract class TokenEmailNotification extends \Piwik\Plugins\UsersManager\TokenNotifications\TokenNotification
{
     ...
     */
    public function __construct(array $tokens, array $recipients, array $emailData)
    {
    ...
    public function dispatch() : bool
    {
        ...
        foreach ($this->recipients as $recipient) {
            LanguagesHelper::doWithUserLanguage($recipient, function () use($recipient, $that) {
                $email = StaticContainer::getContainer()->make($that->getEmailClass(), ['notification' => $that, 'recipient' => $recipient, 'emailData' => $that->emailData[$recipient] ?? []]);
                $email->safeSend();
            });
        }
        return \true;
