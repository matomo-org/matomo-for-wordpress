...
class PagePerformance extends \Piwik\Plugin
{
    ...
    {
        return ['AssetManager.getJavaScriptFiles' => 'getJsFiles', 'Translate.getClientSideTranslationKeys' => 'getClientSideTranslationKeys', 'Actions.Archiving.addActionMetrics' => 'addActionMetrics', 'ScheduledReports.processReports' => 'processReports', 'ViewDataTable.configure' => 'configureViewDataTable', 'Metrics.getDefaultMetricTranslations' => 'addMetricTranslations', 'Metrics.getDefaultMetricSemanticTypes' => 'addMetricSemanticTypes', 'Metrics.isLowerValueBetter' => 'isLowerValueBetter', 'API.Request.dispatch.end' => 'enrichApi'];
    }
    ...
    {
        $metrics = \Piwik\Plugins\PagePerformance\Metrics::getMetricTranslations();
        $translations = array_merge($translations, $metrics);
    }
    public function addMetricSemanticTypes(array &$types) : void
    {
        $metricTypes = \Piwik\Plugins\PagePerformance\Metrics::getMetricSemanticTypes();
    ...
    public function enrichApi($dataTable, $params)
    {
        ...
        $dataTable->filter(function (DataTable $dataTable) {
            ...
            foreach (\Piwik\Plugins\PagePerformance\Metrics::getAllPagePerformanceMetrics() as $pagePerformanceMetric) {
    ...
    {
        \Piwik\Plugins\PagePerformance\Metrics::attachActionMetrics($metricsConfig);
    }
