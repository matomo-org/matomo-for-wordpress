...
class API extends \Piwik\Plugin\API
{
    public function get($idSite, string $period, string $date, ?string $segment = null, $columns = \false)
    {
        ...
        $numericArchives = $archive->getDataTableFromNumeric([\Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_SEARCH_ENGINE_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_SOCIAL_NETWORK_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_AI_ASSISTANT_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_KEYWORD_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_WEBSITE_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_URLS_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_CAMPAIGN_RECORD_NAME]);
        ...
        $dataTable->filter(function (DataTable $table) use($totalVisits) {
            ...
            $percentColumns = ['Referrers_visitorsFromDirectEntry', 'Referrers_visitorsFromSearchEngines', 'Referrers_visitorsFromAIAssistants', 'Referrers_visitorsFromCampaigns', 'Referrers_visitorsFromSocialNetworks', 'Referrers_visitorsFromWebsites'];
    ...
    public function getReferrerType($idSite, string $period, string $date, ?string $segment = null, $typeReferrer = \false, ?int $idSubtable = null, bool $expanded = \false, bool $_setReferrerTypeLabel = \true)
    {
        ...
        $dataTable = $this->getDataTable(\Piwik\Plugins\Referrers\Archiver::REFERRER_TYPE_RECORD_NAME, $idSite, $period, $date, $segment);
        ...
        if ($typeReferrer !== \false && $typeReferrer !== 'false' && $typeReferrer !== '0') {
            // filter for a specific referrer type
    ...
    {
        $idSites = Site::getIdSitesFromIdSitesString($idSite, \false, \true);
    ...
    public function getAll($idSite, string $period, string $date, ?string $segment = null)
    {
        ...
        $dataTable = Request::processRequest('Referrers.getReferrerType', ['idSite' => $idSite, 'period' => $period, 'date' => $date, 'segment' => $segment, 'expanded' => \true, 'disable_generic_filters' => \true, 'disable_queued_filters' => \true, '_setReferrerTypeLabel' => 0], []);
        if ($dataTable instanceof DataTable\Map) {
            throw new Exception("Referrers.getAll with multiple sites or dates is not supported (yet).");
        }
        $dataTable = $dataTable->mergeSubtables($labelColumn = 'referer_type', $useMetadataColumn = \true);
    ...
    public function getKeywords($idSite, string $period, string $date, ?string $segment = null, bool $expanded = \false, bool $flat = \false)
    {
        ...
        if ($flat) {
            ...
            $dataTable->filter('AddSegmentByLabel', ['referrerKeyword', '', $allowEmptyValue = \true]);
    ...
    public function getSearchEngines($idSite, string $period, string $date, ?string $segment = null, bool $expanded = \false, bool $flat = \false)
    {
        ...
        if ($flat) {
            $dataTable->filter('ColumnCallbackAddMetadata', ['label', 'url', function ($url) {
                return \Piwik\Plugins\Referrers\SearchEngine::getInstance()->getUrlFromName($url);
            }]);
            $dataTable->filter('MetadataCallbackAddMetadata', ['url', 'logo', function ($url) {
                return \Piwik\Plugins\Referrers\SearchEngine::getInstance()->getLogoFromUrl($url);
            }]);
            $dataTable->filterSubtables('Piwik\\Plugins\\Referrers\\DataTable\\Filter\\KeywordsFromSearchEngineId', [$dataTable]);
        } else {
            $dataTable->filter('AddSegmentByLabel', ['referrerName']);
            $dataTable->queueFilter('PrependSegment', ['referrerType==search;']);
            $dataTable->queueFilter('ColumnCallbackAddMetadata', ['label', 'url', function ($url) {
                return \Piwik\Plugins\Referrers\SearchEngine::getInstance()->getUrlFromName($url);
            }]);
            $dataTable->queueFilter('MetadataCallbackAddMetadata', ['url', 'logo', function ($url) {
                return \Piwik\Plugins\Referrers\SearchEngine::getInstance()->getLogoFromUrl($url);
            }]);
        }
        return $dataTable;
    ...
    public function getUrlsFromWebsiteId($idSite, string $period, string $date, int $idSubtable, ?string $segment = null)
    {
        ...
        $dataTable->filter('MetadataCallbackAddMetadata', ['url', 'segment', function ($url) {
    ...
    public function getSocials($idSite, string $period, string $date, ?string $segment = null, bool $expanded = \false, bool $flat = \false)
    {
        ...
        $dataTable->filter('ColumnCallbackAddMetadata', ['label', 'url', function ($name) {
            return \Piwik\Plugins\Referrers\Social::getInstance()->getMainUrlFromName($name);
        }]);
        $dataTable = $this->completeSocialTablesWithOldReports($dataTable, $idSite, $period, $date, $segment, $expanded, $flat);
        $dataTable->filter('MetadataCallbackAddMetadata', ['url', 'logo', function ($url) {
            return \Piwik\Plugins\Referrers\Social::getInstance()->getLogoFromUrl($url);
        }]);
        $dataTable->filter('AddSegmentByLabel', ['referrerName']);
    ...
    protected function filterWebsitesForSocials($dataTable, $idSite, string $period, string $date, ?string $segment, bool $expanded, bool $flat) : void
    {
        ...
        $dataTable->filter('ColumnCallbackAddMetadata', ['label', 'url', function ($url) {
            return \Piwik\Plugins\Referrers\Social::getInstance()->getMainUrl($url);
        }]);
        $dataTable->filter('GroupBy', ['label', function ($url) {
            return \Piwik\Plugins\Referrers\Social::getInstance()->getSocialNetworkFromDomain($url);
        }]);
    ...
    protected function filterWebsitesForAIAssistants($dataTable) : void
    {
        ...
        $dataTable->filter('ColumnCallbackAddMetadata', ['label', 'url', function ($url) {
            return \Piwik\Plugins\Referrers\AIAssistant::getInstance()->getMainUrl($url);
        }]);
        $dataTable->filter('GroupBy', ['label', function ($url) {
    ...
    private function setSocialIdSubtables($dataTable) : void
    {
        $dataTable->filter(function (DataTable $table) {
            foreach ($table->getRows() as $row) {
                ...
                foreach (\Piwik\Plugins\Referrers\Social::getInstance()->getDefinitions() as $name) {
                    if ($name == $socialName) {
        ...
        foreach ($dataTables as $label => $dataTable) {
            foreach ($dataTable->getRows() as $row) {
                ...
                foreach ($rows as $id => $urlsTableRow) {
                    ...
                    if (\Piwik\Plugins\Referrers\Social::getInstance()->isSocialUrl($url, $social)) {
    ...
    private function createReferrerTypeTable(DataTable\DataTableInterface $table)
    {
        if ($table instanceof DataTable) {
            $nameToColumnId = [Common::REFERRER_TYPE_SEARCH_ENGINE => 'Referrers_visitorsFromSearchEngines', Common::REFERRER_TYPE_SOCIAL_NETWORK => 'Referrers_visitorsFromSocialNetworks', Common::REFERRER_TYPE_AI_ASSISTANT => 'Referrers_visitorsFromAIAssistants', Common::REFERRER_TYPE_DIRECT_ENTRY => 'Referrers_visitorsFromDirectEntry', Common::REFERRER_TYPE_WEBSITE => 'Referrers_visitorsFromWebsites', Common::REFERRER_TYPE_CAMPAIGN => 'Referrers_visitorsFromCampaigns'];
