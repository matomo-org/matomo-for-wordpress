...
class API extends \Piwik\Plugin\API
{
    public function get($idSite, $period, $date, $segment = false, $columns = false)
    {
        ...
        $numericArchives = $archive->getDataTableFromNumeric([\Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_SEARCH_ENGINE_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_SOCIAL_NETWORK_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_KEYWORD_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_WEBSITE_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_URLS_RECORD_NAME, \Piwik\Plugins\Referrers\Archiver::METRIC_DISTINCT_CAMPAIGN_RECORD_NAME]);
        ...
        $dataTable->filter(function (DataTable $table) use($totalVisits) {
            ...
            $percentColumns = ['Referrers_visitorsFromDirectEntry', 'Referrers_visitorsFromSearchEngines', 'Referrers_visitorsFromCampaigns', 'Referrers_visitorsFromSocialNetworks', 'Referrers_visitorsFromWebsites'];
    ...
    public function getReferrerType($idSite, $period, $date, $segment = false, $typeReferrer = false, $idSubtable = false, $expanded = false, $_setReferrerTypeLabel = true)
    {
        ...
        $dataTable->filter('AddSegmentByLabelMapping', array('referrerType', array(Common::REFERRER_TYPE_DIRECT_ENTRY => 'direct', Common::REFERRER_TYPE_CAMPAIGN => 'campaign', Common::REFERRER_TYPE_SEARCH_ENGINE => 'search', Common::REFERRER_TYPE_SOCIAL_NETWORK => 'social', Common::REFERRER_TYPE_WEBSITE => 'website')));
    ...
    public function getAll($idSite, $period, $date, $segment = false)
    {
        ...
        $dataTable = Request::processRequest('Referrers.getReferrerType', ['idSite' => $idSite, 'period' => $period, 'date' => $date, 'segment' => $segment, 'expanded' => true, 'disable_generic_filters' => true, 'disable_queued_filters' => true, '_setReferrerTypeLabel' => 0], []);
    ...
    {
        Piwik::checkUserHasViewAccess($idSite);
        $dataTable = $this->getDataTable(\Piwik\Plugins\Referrers\Archiver::SOCIAL_NETWORKS_RECORD_NAME, $idSite, $period, $date, $segment, $expanded = true, $idSubtable);
        if (!$idSubtable) {
        ...
        $dataTable = $this->combineDataTables($dataTable, function () use($idSite, $period, $date, $segment, $idSubtable) {
            $dataTableFiltered = $this->getDataTable(\Piwik\Plugins\Referrers\Archiver::WEBSITES_RECORD_NAME, $idSite, $period, $date, $segment, $expanded = true);
            ...
            $dataTableFiltered->filter('ColumnCallbackDeleteRow', array('label', function ($url) use($social) {
    ...
    {
        $urlsTable = Archive::createDataTableFromArchive(\Piwik\Plugins\Referrers\Archiver::WEBSITES_RECORD_NAME, $idSite, $period, $date, $segment, $expanded, $flat = true);
    ...
    {
        if ($table instanceof DataTable) {
            $nameToColumnId = array(Common::REFERRER_TYPE_SEARCH_ENGINE => 'Referrers_visitorsFromSearchEngines', Common::REFERRER_TYPE_SOCIAL_NETWORK => 'Referrers_visitorsFromSocialNetworks', Common::REFERRER_TYPE_DIRECT_ENTRY => 'Referrers_visitorsFromDirectEntry', Common::REFERRER_TYPE_WEBSITE => 'Referrers_visitorsFromWebsites', Common::REFERRER_TYPE_CAMPAIGN => 'Referrers_visitorsFromCampaigns');
