...
class Controller extends \Piwik\Plugin\Controller
{
    ...
    private function assertUrlTokenAuthDidNotFail() : void
    {
        ...
        if (GeneralConfig::getBoolConfigValue('only_allow_secure_auth_tokens', \false)) {
            $message = Piwik::translate('Widgetize_ErrorTokenAuthFailedDisabledByPolicy', ['<a href="index.php?module=UsersManager" rel="noreferrer noopener" target="_blank">', '</a>']);
        } else {
            $message = Piwik::translate('Widgetize_ErrorTokenAuthFailed', ['<a href="index.php?module=UsersManager&action=userSecurity" rel="noreferrer noopener" target="_blank">', '</a>', '<a href="index.php?module=UsersManager" rel="noreferrer noopener" target="_blank">', '</a>']);
        }
        $ex = new \Piwik\Plugins\Widgetize\UrlTokenAuthFailedException($message);
        $ex->setIsHtmlMessage();
        throw $ex;
    }
    private function assertDispatchedContentIsEmbeddable(string $module, string $action) : void
    {
        ...
        if ($contentType !== '' && stripos($contentType, 'text/html') === \false && stripos($contentType, 'application/xhtml') === \false) {
            throw new \Exception(sprintf("%s.%s cannot be widgetized: only actions returning HTML can be embedded.", $module, $action));
        }
    }
