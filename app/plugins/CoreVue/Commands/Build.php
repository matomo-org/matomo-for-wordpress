...
class Build extends ConsoleCommand
{
    ...
    protected function doExecute() : int
    {
        ...
        if (empty($plugins)) {
            ...
            $output->writeln("<info>Going to build all plugins with Vue libraries: " . implode(', ', $plugins));
        } else {
    ...
    {
        $commandSingle = 'cd ' . PIWIK_INCLUDE_PATH . ' && ' . "BROWSERSLIST_IGNORE_OLD_DATA=1 FORCE_COLOR=1 MATOMO_CURRENT_PLUGIN=%2\$s " . 'MATOMO_ALL_PLUGINS=' . implode(',', $allPluginNames) . ' ' . 'node ' . self::getVueCliServiceProxyBin() . ' build --mode=development --target lib --name ' . "%1\$s --filename=%1\$s.development --no-clean %2\$s/vue/src/index.ts --dest %2\$s/vue/dist --watch &";
        $command = '';
        foreach ($plugins as $plugin) {
    ...
    private function buildFiles(string $plugin, bool $printBuildCommand, array $allPluginNames) : int
    {
        $output = $this->getOutput();
        $pluginDirPath = Manager::getRelativePluginDirectory($plugin);
        $command = 'cd ' . PIWIK_INCLUDE_PATH . ' && ' . "BROWSERSLIST_IGNORE_OLD_DATA=1 FORCE_COLOR=1 MATOMO_CURRENT_PLUGIN={$pluginDirPath} " . 'MATOMO_ALL_PLUGINS=' . implode(',', $allPluginNames) . ' ' . 'node ' . self::getVueCliServiceProxyBin() . ' build --target lib --name ' . $plugin . " {$pluginDirPath}/vue/src/index.ts --dest {$pluginDirPath}/vue/dist";
        if ($printBuildCommand) {
        ...
        if ($output->isVerbose()) {
            ...
            while ($attempts < self::RETRY_COUNT) {
                ...
                if ($this->isTypeScriptRaceConditionInOutput($plugin, $concattedOutput)) {
                    $output->writeln("<comment>The TypeScript compiler encountered a race condition when compiling " . "files (files that exist were not found), retrying.</comment>");
                    ++$attempts;
                    continue;
                }
    ...
    private function checkNodeJsVersion() : void
    {
        ...
        if (version_compare($nodeVersion, self::RECOMMENDED_NODE_VERSION, '<')) {
            $output->writeln(sprintf("<comment>The recommended node version for working with Vue is version %s or " . "greater and it looks like you're using %s. Building Vue files may not work with an older version, so " . "we recommend upgrading. nvm can be used to easily install new node versions.</comment>", self::RECOMMENDED_NODE_VERSION, $nodeVersion));
        }
        if (version_compare($npmVersion, self::RECOMMENDED_NPM_VERSION, '<')) {
            $output->writeln(sprintf("<comment>The recommended npm version for working with Vue is version %s " . "or greater and it looks like you're using %s. Using an older version may result in improper " . "dependencies being used, so we recommend upgrading. You can upgrade to the latest version with the " . "command %s</comment>", self::RECOMMENDED_NPM_VERSION, $npmVersion, 'npm install -g npm@latest'));
        }
    }
    private function isTypeScriptRaceConditionInOutput(string $plugin, string $concattedOutput) : bool
    {
        ...
        $matches = [];
        preg_match_all('/in\\s+([^\\n]+)\\(\\d+,\\d+\\)\\s*\\n\\s*TS2307:\\s+Cannot find module \'([^\']+)\' or its corresponding type declarations\\./m', $normalizedOutput, $matches, \PREG_SET_ORDER);
    ...
    {
        $pluginDirectories = array_merge($GLOBALS['MATOMO_PLUGIN_DIRS'] ?? [], [['pluginsPathAbsolute' => PIWIK_INCLUDE_PATH . '/plugins', 'webrootDirRelativeToMatomo' => '.']]);
        $allPlugins = [];
