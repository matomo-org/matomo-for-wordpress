...
/**
 */
class Validate extends \Piwik\Plugins\LanguagesManager\Commands\TranslationBase
{
    protected function configure()
    {
        $this->setName('translations:validate')->setDescription('Validates translation files')->addOptionalValueOption('token', 't', 'Weblate API token')->addOptionalValueOption('slug', 's', 'Weblate project slug')->addNoValueOption('all', 'a', 'Force to update all plugins (even non core). Can not be used with plugin option')->addOptionalValueOption('plugin', 'P', 'optional name of plugin to update translations for');
    }
    protected function doExecute() : int
    {
        ...
        foreach ($pluginList as $plugin) {
            ...
            foreach ($files as $filename) {
                $this->runCommand('translations:set', ['--code' => basename($filename, '.json'), '--file' => $filename, '--plugin' => $plugin, '--validate' => PIWIK_DOCUMENT_ROOT . '/filter.txt']);
            }
            $output->writeln('');
        }
    ...
    {
        $input = $this->getInput();
        $this->runCommand('translations:fetch', ['--token' => $input->getOption('token'), '--slug' => $input->getOption('slug'), '--plugin' => $plugin]);
    }
}
