...
class Menu extends \Piwik\Plugin\Menu
{
    ...
    private function addContainerMenuItems(\Piwik\Plugins\TagManager\MenuTagManager $menu, array $containers, string $action) : void
    {
        foreach ($containers as $container) {
            $params = array('idContainer' => $container['idcontainer']);
            $menu->addItem('TagManager_TagManager', self::CONTAINER_MENU_ITEM_KEY_PREFIX . $container['idcontainer'], $this->urlForAction($action, $params), 100, $container['name'], \false, \false, \false, \false, 0, 'container-menu-item');
        }
    }
    public function configureTagManagerMenu(\Piwik\Plugins\TagManager\MenuTagManager $menu)
    {
        ...
        $manageContainersParams = array('idContainer' => $currentIdContainer ?: null);
        ...
        if (!$action || $action === 'manageContainers') {
            $action = $this->accessValidator->hasWriteCapability($idSite) ? 'dashboard' : 'manageTags';
        }
