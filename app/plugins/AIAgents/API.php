...
class API extends PluginAPI
{
    ...
    public function get($idSite, string $period, string $date, string $segment = '', $columns = '') : DataTableInterface
    {
        ...
        $clientTypes = [self::AI_AGENT_COLUMN_SUFFIX => self::AI_AGENT_SEGMENT, self::HUMAN_COLUMN_SUFFIX => self::HUMAN_SEGMENT];
        foreach ($clientTypes as $columnSuffix => $clientTypeSegment) {
            ...
            $params = ['idSite' => $idSite, 'period' => $period, 'date' => $date, 'segment' => $modifiedSegment, 'columns' => implode(',', $columnsForClientType), 'format' => 'original', 'format_metrics' => 0];
