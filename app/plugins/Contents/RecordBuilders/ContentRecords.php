...
class ContentRecords extends RecordBuilder
{
    ...
    {
        return [Record::make(Record::TYPE_BLOB, Archiver::CONTENTS_NAME_PIECE_RECORD_NAME), Record::make(Record::TYPE_BLOB, Archiver::CONTENTS_PIECE_NAME_RECORD_NAME)];
    }
    protected function aggregate(ArchiveProcessor $archiveProcessor) : array
    {
        $reports = [Archiver::CONTENTS_PIECE_NAME_RECORD_NAME => new DataTable(), Archiver::CONTENTS_NAME_PIECE_RECORD_NAME => new DataTable()];
    ...
    private function aggregateImpressionRow(array $reports, $row) : void
    {
        foreach ($this->getRecordToDimensions() as $record => $dimensions) {
            ...
            $columns = [Metrics::INDEX_NB_UNIQ_VISITORS => $row[Metrics::INDEX_NB_UNIQ_VISITORS], Metrics::INDEX_NB_VISITS => $row[Metrics::INDEX_NB_VISITS], Metrics::INDEX_CONTENT_NB_IMPRESSIONS => $row[Metrics::INDEX_CONTENT_NB_IMPRESSIONS], Metrics::INDEX_CONTENT_NB_INTERACTIONS => 0];
            // content piece is optional
            if ($mainDimension == 'contentPiece' && empty($mainLabel)) {
            ...
            // content piece is optional
            if ($subDimension == 'contentPiece' && empty($subLabel)) {
    ...
    private function aggregateInteractionRow(array $reports, array $row) : void
    {
        foreach ($this->getRecordToDimensions() as $record => $dimensions) {
            ...
            $subDimension = $dimensions[1];
            $subLabel = $row[$subDimension];
            $columns = [Metrics::INDEX_CONTENT_NB_INTERACTIONS => $row[Metrics::INDEX_CONTENT_NB_INTERACTIONS]];
    ...
    {
        return array(Archiver::CONTENTS_PIECE_NAME_RECORD_NAME => array('contentPiece', 'contentName'), Archiver::CONTENTS_NAME_PIECE_RECORD_NAME => array('contentName', 'contentPiece'));
    }
}
