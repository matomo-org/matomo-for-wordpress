...
abstract class GridGraph extends StaticGraph
{
    ...
    public const VERTICAL_LEGEND_MAX_WIDTH_PCT = 0.7;
    ...
    {
        return array(self::GRAPHIC_COLOR_KEY . '1' => '5170AE', self::GRAPHIC_COLOR_KEY . '2' => 'F29007', self::GRAPHIC_COLOR_KEY . '3' => 'CC3399', self::GRAPHIC_COLOR_KEY . '4' => '9933CC', self::GRAPHIC_COLOR_KEY . '5' => '80A033', self::GRAPHIC_COLOR_KEY . '6' => '246AD2');
    }
    protected function initGridChart($displayVerticalGridLines, $bulletType, $horizontalGraph, $showTicks, $verticalLegend)
    {
        $this->initpData();
        $colorIndex = 1;
        ...
        $this->drawBackground();
        $this->pImage->setGraphArea($topLeftXValue, $topLeftYValue, $bottomRightXValue, $bottomRightYValue);
        ...
        if (!$horizontalGraph) {
            ...
            if ($this->forceSkippedLabels && $skippedLabels && $skippedLabels < $this->forceSkippedLabels && $abscissaSeriesCount > $this->forceSkippedLabels + 1) {
                $skippedLabels = $this->forceSkippedLabels;
            }
        }
        $ordinateAxisLength = $horizontalGraph ? $bottomRightXValue - $topLeftXValue : $this->getGraphHeight($horizontalGraph, $verticalLegend);
        $maxOrdinateValue = 0;
        foreach ($this->ordinateSeries as $column => $data) {
        ...
        if ($this->showLegend) {
            ...
            if (count($this->ordinateLabels) > 1) {
                ...
                foreach ($this->ordinateLabels as $metricCode => &$label) {
                    ...
                    $this->pImage->drawFilledRectangle($rectangleTopLeftXValue, $rectangleTopLeftYValue, $rectangleBottomRightXValue, $rectangleBottomRightYValue, array('Alpha' => self::LEGEND_SHADOW_OPACITY, 'R' => $color['R'], 'G' => $color['G'], 'B' => $color['B']));
                    if ($verticalLegend) {
            ...
            $this->pImage->drawLegend($legendTopLeftXValue, $legendTopLeftYValue, array(
                'Style' => LEGEND_NOBORDER,
                'FontSize' => $this->legendFontSize,
                'BoxWidth' => $bulletWidth,
                'XSpacing' => $effectiveLegendItemHorizontalInterstice,
                // not effective when vertical
