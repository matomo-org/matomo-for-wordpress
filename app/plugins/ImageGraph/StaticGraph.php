...
abstract class StaticGraph extends BaseFactory
{
    ...
    {
        return Piwik::translate('General_ExceptionInvalidStaticGraphType', array($graphType, implode(', ', self::getAvailableStaticGraphTypes())));
    }
    ...
    public function setFontSize($fontSize)
    {
        if (!is_numeric($fontSize)) {
            $fontSize = \Piwik\Plugins\ImageGraph\API::DEFAULT_FONT_SIZE;
        }
    ...
    protected function createResizedImageCopyIfNeeded($image)
    {
        ...
        if ($size[0] <= 16 && $size[1] <= 16) {
            return $image;
            // use original image if size fits
        }
        $ratio = $size[0] / $size[1];
        if ($ratio > 1) {
            $width = 16;
    ...
    {
        $this->pImage->drawFilledRectangle(0, 0, $this->width, $this->height, array_merge(array('Alpha' => 100), $this->backgroundColor));
    }
    private static function hex2rgb($hexColor)
    {
        if (preg_match('/([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/', $hexColor, $matches)) {
            return array('R' => hexdec($matches[1]), 'G' => hexdec($matches[2]), 'B' => hexdec($matches[3]));
        } else {
            return [];
        }
    }
