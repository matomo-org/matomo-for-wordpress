...
class Extraction
{
    ...
    public function check()
    {
        ...
        if (!empty($this->pattern) && $this->dimension !== 'urlparam') {
            ...
            if (1 !== substr_count($this->pattern, '(') - $ncgCount || 1 !== substr_count($this->pattern, ')') - $ncgCount || 1 !== substr_count($this->pattern, ')', strpos($this->pattern, '(')) - $ncgCount) {
    ...
    {
        return array('url' => Piwik::translate('Actions_ColumnPageURL'), 'urlparam' => Piwik::translate('CustomDimensions_PageUrlParam'), 'action_name' => Piwik::translate('Goals_PageTitle'));
    }
