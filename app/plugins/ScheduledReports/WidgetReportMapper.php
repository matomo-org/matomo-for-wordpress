...
class WidgetReportMapper
{
    ...
    public const NO_REPORT_WIDGETS = ['widgetTourgetEngagement', 'widgetMarketplacegetPremiumFeatures', 'widgetRssWidgetrssPiwik', 'widgetRssWidgetrssChangelog', 'widgetProfessionalServicespromoServices', 'widgetInstallationgetSystemCheck', 'widgetCoreHomequickLinks', 'widgetCoreHomegetSystemSummary', 'widgetCoreHomegetPromoVideo', 'widgetMarketplacegetNewPlugins', 'widgetReferrersgetCampaignUrlBuilder', 'widgetCoreHomegetDonateForm'];
    /**
    ...
    public function getMappingForSite(string $idSite) : array
    {
        ...
        foreach ($this->getWidgetConfigs() as $widgetConfig) {
            ...
            $reportId = $reportId ?? $this->guessReportIdFromHeuristics($widgetModule, $widgetAction, $reportIndex, $parameters);
     ...
     */
    private function findReportIdByWidgetParameters(string $widgetKey, array $parameters, array $reportIndex) : ?string
    {
    ...
    {
        $namesById = [];
        $widgetIdLookup = array_fill_keys($widgetIds, \true);
        foreach ($this->getWidgetConfigs() as $widgetConfig) {
            $uniqueId = $widgetConfig->getUniqueId();
            if (in_array($uniqueId, self::NO_REPORT_WIDGETS, \true)) {
                continue;
            }
            ...
            if (!empty($translatedCategoryName) && stripos($translatedWidgetName, $translatedCategoryName) !== 0) {
     ...
     * @param array<string, mixed> $parameters
     */
    private function guessReportIdFromHeuristics(string $module, string $action, array $reportIndex, array $parameters = []) : ?string
    {
        if ('getEvolutionGraph' !== $action) {
            return null;
        }
        if ($module === 'CustomReports') {
            $reportId = $this->findReportIdByWidgetParameters('CustomReports.getCustomReport', $parameters, $reportIndex);
            if ($reportId !== null) {
                return $reportId;
            }
        }
        $fallbackKey = $module . '.get';
        $reportId = $this->findReportIdByWidgetParameters($fallbackKey, $parameters, $reportIndex);
        if ($reportId !== null) {
            return $reportId;
        }
