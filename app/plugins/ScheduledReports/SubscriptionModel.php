...
class SubscriptionModel
{
    ...
    public function unsubscribe($token)
    {
        ...
        if ($emailFound) {
            $reportModel = new \Piwik\Plugins\ScheduledReports\Model();
            $reportModel->updateReport($report['idreport'], array('parameters' => json_encode($report['parameters'])));
    ...
    public function updateReportSubscriptions($idReport, $emails)
    {
        ...
        foreach ($emails as $email) {
            ...
            if (!in_array($email, $availableEmails)) {
                $subscription = ['idreport' => $idReport, 'token' => $token, 'email' => $email];
