...
abstract class GenerateVueConstructBase extends \Piwik\Plugins\CoreConsole\Commands\GeneratePluginBase
{
    /**
    ...
    protected function getConstructName($optionName, $constructType)
    {
        ...
        $validate = function ($testname) use($constructType) {
            ...
            if (!ctype_alnum($testname)) {
                throw new \InvalidArgumentException("Only alphanumeric characters are allowed as a {$constructType} " . "name. Use CamelCase if the name of your {$constructType} contains multiple words.");
            }
            return $testname;
        };
        if (empty($testname)) {
            $testname = $this->askAndValidate("Enter the name of the {$constructType} you want to create: ", $validate);
        } else {
            $validate($testname);
        }
