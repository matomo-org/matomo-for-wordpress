...
class GenerateReport extends \Piwik\Plugins\CoreConsole\Commands\GeneratePluginBase
{
    protected function configure()
    {
        $this->setName('generate:report')->setDescription('Adds a new report to an existing plugin')->addRequiredValueOption('pluginname', null, 'The name of an existing plugin which does not have a menu defined yet')->addRequiredValueOption('reportname', null, 'The name of the report you want to create')->addRequiredValueOption('category', null, 'The name of the category the report belongs to')->addOptionalValueOption('dimension', null, 'The name of the dimension in case your report has a dimension')->addRequiredValueOption('documentation', null, 'A documentation that explains what your report is about');
    }
    protected function doExecute() : int
    {
        ...
        $replace = array('GetExampleReport' => ucfirst($apiName), 'getExampleReport' => lcfirst($apiName), 'getApiReport' => lcfirst($apiName), 'ExampleCategory' => $category, 'ExampleReportName' => $this->makeTranslationIfPossible($pluginName, $reportName), 'ExampleReportDocumentation' => $documentation, '999' => $order, 'new ExitPageUrl()' => $dimension, 'use Piwik\\Plugins\\Actions\\Columns\\ExitPageUrl;' => $dimensionClass, 'ExampleReport' => $pluginName);
    ...
    protected function getReportName()
    {
        ...
        if (empty($reportName)) {
            $reportName = $this->askAndValidate('Enter the name of your report, for instance "Browser Families": ', $validate);
        } else {
            $validate($reportName);
        }
    ...
    protected function getDocumentation()
    {
        ...
        if (empty($documentation)) {
            $documentation = $this->askAndValidate('Enter a documentation that describes the data of your report (you can leave it empty and define it later): ', $validate);
        } else {
    ...
    protected function getDimension($pluginName)
    {
        ...
        if (null === $actualDimension) {
            $actualDimension = $this->askAndValidate('Enter the report dimension, for instance "Browser" (you can leave it either empty or use an existing one): ', $validate, null, $dimensionNames);
        } else {
