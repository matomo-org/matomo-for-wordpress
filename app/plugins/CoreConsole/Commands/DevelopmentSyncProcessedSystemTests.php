...
class DevelopmentSyncProcessedSystemTests extends ConsoleCommand
{
    ...
    protected function updateCoreFiles()
    {
        ...
        if ($tar->extract($targetDir)) {
            $this->writeSuccessMessage(array('All processed system test results were copied to <comment>' . $targetDir . '</comment>', 'Compare them with the expected test results and commit them if needed.'));
        } else {
