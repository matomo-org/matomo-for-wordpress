...
class Controller extends ControllerAdmin
{
    ...
    private function setManageVariables(View $view)
    {
        ...
        $mobileMessagingAPI = \Piwik\Plugins\MobileMessaging\API::getInstance();
        $model = new \Piwik\Plugins\MobileMessaging\Model();
        ...
        foreach ($this->regionDataProvider->getCountryList() as $countryCode => $continentCode) {
            if (isset(\Piwik\Plugins\MobileMessaging\CountryCallingCodes::$countryCallingCodes[$countryCode])) {
                if ($countryCode == $defaultCountry) {
                    $view->defaultCallingCode = \Piwik\Plugins\MobileMessaging\CountryCallingCodes::$countryCallingCodes[$countryCode];
                }
                $countries[] = array('key' => \Piwik\Plugins\MobileMessaging\CountryCallingCodes::$countryCallingCodes[$countryCode], 'value' => \Piwik\Plugins\UserCountry\countryTranslate($countryCode));
            }
        }
