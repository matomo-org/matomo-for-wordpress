...
class ASPSMS extends SMSProvider
{
    ...
    {
        return array(array('type' => 'text', 'name' => 'username', 'title' => 'MobileMessaging_UserKey'), array('type' => 'text', 'name' => 'password', 'title' => 'General_Password'));
    }
    ...
    public function sendSMS($credentials, $smsText, $phoneNumber, $from)
    {
        ...
        $additionalParameters = array('Recipients' => array($phoneNumber), 'MessageText' => $smsText, 'Originator' => $from, 'AffiliateID' => '227830');
        $this->issueApiCall($credentials, self::SEND_SMS_RESOURCE, $additionalParameters);
    }
    private function issueApiCall($credentials, $resource, $additionalParameters = array())
    {
        $accountParameters = array('UserName' => $credentials['username'], 'Password' => $credentials['password']);
    ...
    {
        $credits = $this->issueApiCall($credentials, self::CHECK_CREDIT_RESOURCE);
