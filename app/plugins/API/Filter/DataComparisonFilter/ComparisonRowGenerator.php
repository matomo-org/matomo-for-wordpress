...
class ComparisonRowGenerator
{
    ...
    private function compareTable($compareMetadata, DataTable $table, DataTable $rootCompareTable = null, DataTable $compareTable = null)
    {
        ...
        if ($compareTable) {
            ...
            if (!empty($totals)) {
                ...
                $comparisonTotalsEntry = array_merge($compareMetadata, ['totals' => $totals]);
    ...
    private function compareRow(DataTable $table, $compareMetadata, DataTable\Row $row, DataTable\Row $compareRow = null, DataTable $rootTable = null)
    {
        ...
        if (empty($comparisonDataTable)) {
            ...
            $comparisonDataTable->setMetadata(DataTable::EXTRA_PROCESSED_METRICS_METADATA_NAME, $table->getMetadata(DataTable::EXTRA_PROCESSED_METRICS_METADATA_NAME));
        ...
        if ($compareRow) {
            foreach ($compareRow as $name => $value) {
                if (!is_numeric($value) || $name == 'label') {
                    continue;
                }
                $columns[$name] = $value;
            }
        ...
        $newRow = new DataTable\Row([DataTable\Row::COLUMNS => $columns, DataTable\Row::METADATA => $compareMetadata]);
        // set subtable
