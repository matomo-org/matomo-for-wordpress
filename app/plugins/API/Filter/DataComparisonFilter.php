...
class DataComparisonFilter
{
    ...
    {
        $params = array_merge(['filter_limit' => -1, 'filter_offset' => 0, 'filter_sort_column' => '', 'filter_truncate' => -1, 'compare' => 0, 'totals' => 1, 'disable_queued_filters' => 1, 'format_metrics' => 0, 'label' => '', 'flat' => $this->request->getIntegerParameter('flat', 0), 'filter_add_columns_when_show_all_columns' => $this->request->getStringParameter('filter_add_columns_when_show_all_columns', ''), 'filter_update_columns_when_show_all_goals' => $this->request->getStringParameter('filter_update_columns_when_show_all_goals', ''), 'filter_show_goal_columns_process_goals' => $this->request->getStringParameter('filter_show_goal_columns_process_goals', ''), 'idGoal' => $this->request->getStringParameter('idGoal', '')], $paramsToModify);
        ...
        if ($idSubtable > 0) {
            ...
            if ($comparisonIdSubtable === -1) {
                // no subtable in comparison row
                $table = new DataTable();
    ...
    private function getMetadataFromModifiedParams($modifiedParams)
    {
        ...
        $metadata['compareSeriesPretty'] = self::getComparisonSeriesLabelSuffixFromParts($metadata['comparePeriodPretty'], $metadata['compareSegmentPretty']);
        return $metadata;
    }
    private static function getComparisonSeriesLabelSuffixFromParts($periodPretty, $segmentPretty)
    {
        $comparisonLabels = [$periodPretty, $segmentPretty];
