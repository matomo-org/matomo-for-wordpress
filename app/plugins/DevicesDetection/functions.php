...
function getBrowserLogo($short)
{
    ...
    $browserFamilies = BrowserParser::getAvailableBrowserFamilies();
    if (array_key_exists($short, BrowserParser::getAvailableBrowsers()) && file_exists(PIWIK_INCLUDE_PATH . '/' . sprintf($path, $short))) {
...
{
    return [1 => 'browser', 2 => 'library', 3 => 'feed reader', 4 => 'mediaplayer', 5 => 'mobile app', 6 => 'pim'];
}
function getClientTypeLabel($label)
{
    $translations = ['browser' => 'DevicesDetection_ColumnBrowser', 'library' => 'DevicesDetection_Library', 'feed reader' => 'DevicesDetection_FeedReader', 'mediaplayer' => 'DevicesDetection_MediaPlayer', 'mobile app' => 'DevicesDetection_MobileApp', 'pim' => 'DevicesDetection_Pim'];
    $clientTypes = getClientTypeMapping();
    if (is_numeric($label) && array_key_exists($label, $clientTypes) && isset($translations[$clientTypes[$label]])) {
...
{
    $translations = ['desktop' => 'General_Desktop', 'smartphone' => 'DevicesDetection_Smartphone', 'tablet' => 'DevicesDetection_Tablet', 'phablet' => 'DevicesDetection_Phablet', 'feature phone' => 'DevicesDetection_FeaturePhone', 'console' => 'DevicesDetection_Console', 'tv' => 'DevicesDetection_TV', 'car browser' => 'DevicesDetection_CarBrowser', 'smart display' => 'DevicesDetection_SmartDisplay', 'camera' => 'DevicesDetection_Camera', 'portable media player' => 'DevicesDetection_PortableMediaPlayer', 'smart speaker' => 'DevicesDetection_SmartSpeaker', 'wearable' => 'DevicesDetection_Wearable', 'peripheral' => 'DevicesDetection_Peripheral'];
...
function getBrowserEngineName($engineName)
{
     ...
     */
    $oldTypeMapping = ['ie' => 'Trident', 'gecko' => 'Gecko', 'khtml' => 'KHTML', 'webkit' => 'WebKit', 'opera' => 'Presto', 'unknown' => ''];
    if (array_key_exists($engineName, $oldTypeMapping)) {
        $engineName = $oldTypeMapping[$engineName];
    }
    $displayNames = ['Trident' => 'Trident (IE)', 'Gecko' => 'Gecko (Firefox)', 'KHTML' => 'KHTML (Konqueror)', 'Presto' => 'Presto (Opera)', 'WebKit' => 'WebKit (Safari)', 'Blink' => 'Blink (Chrome, Opera)'];
    if (!empty($engineName)) {
