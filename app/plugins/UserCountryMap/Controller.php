...
class Controller extends \Piwik\Plugin\Controller
{
    ...
    public function visitorMap($fetch = \false, $segmentOverride = \false)
    {
        ...
        $view->reqParamsJSON = $this->getEnrichedRequest($params = ['period' => $period, 'idSite' => $this->idSite, 'date' => $date, 'segment' => $segment, 'token_auth' => $token_auth, 'enable_filter_excludelowpop' => 1, 'filter_excludelowpop_value' => -1]);
        ...
        $view->continents = ['AF' => \Piwik\Plugins\UserCountry\continentTranslate('afr'), 'AS' => \Piwik\Plugins\UserCountry\continentTranslate('asi'), 'EU' => \Piwik\Plugins\UserCountry\continentTranslate('eur'), 'NA' => \Piwik\Plugins\UserCountry\continentTranslate('amn'), 'OC' => \Piwik\Plugins\UserCountry\continentTranslate('oce'), 'SA' => \Piwik\Plugins\UserCountry\continentTranslate('ams')];
        return $view->render();
    }
    /**
    ...
    public function realtimeMap($standalone = \false, $fetch = \false, $segmentOverride = \false)
    {
        ...
        // some translations
        $locale = ['nb_actions' => $this->translator->translate('VisitsSummary_NbActionsDescription'), 'local_time' => $this->translator->translate('VisitTime_ColumnLocalTime'), 'from' => $this->translator->translate('General_FromReferrer'), 'seconds' => $this->translator->translate('Intl_Seconds'), 'seconds_ago' => $this->translator->translate('UserCountryMap_SecondsAgo'), 'minutes' => $this->translator->translate('Intl_Minutes'), 'minutes_ago' => $this->translator->translate('UserCountryMap_MinutesAgo'), 'hours' => $this->translator->translate('Intl_Hours'), 'hours_ago' => $this->translator->translate('UserCountryMap_HoursAgo'), 'days_ago' => $this->translator->translate('UserCountryMap_DaysAgo'), 'actions' => $this->translator->translate('Transitions_NumPageviews'), 'searches' => $this->translator->translate('UserCountryMap_Searches'), 'goal_conversions' => $this->translator->translate('UserCountryMap_GoalConversions')];
        $segment = ($segmentOverride ?: Request::getRawSegmentFromRequest()) ?: '';
        $params = ['period' => 'range', 'idSite' => $this->idSite, 'segment' => $segment, 'token_auth' => $token_auth];
    ...
    private function getMetrics($idSite, $period, $date, #[\SensitiveParameter] $token_auth)
    {
    ...
    private function getApiRequestUrl($module, $action, $idSite, $period, $date, #[\SensitiveParameter] $token_auth, $filter_by_country = \false, $segmentOverride = \false)
    {
    ...
    private function report($module, $action, $idSite, $period, $date, #[\SensitiveParameter] $token_auth, $filter_by_country = \false, $segmentOverride = \false)
    {
