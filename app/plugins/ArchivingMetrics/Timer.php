...
final class Timer
{
    ...
    public function start(\Piwik\Plugins\ArchivingMetrics\Context $context) : void
    {
        if (\false === $this->isApplicableForTiming($context)) {
            return;
        }
        $this->runs[$context->getKey()] = ['context' => $context, 'timeStarted' => $this->clock->microtime()];
    }
    public function complete(\Piwik\Plugins\ArchivingMetrics\Context $context, array $idArchives, bool $wasCached) : void
    {
        if (\false === $this->isApplicableForTiming($context)) {
            return;
        }
    ...
    private function isApplicableForTiming(\Piwik\Plugins\ArchivingMetrics\Context $context) : bool
    {
        ...
        $doneFlag = Rules::getDoneStringFlagFor([$context->idSite], $context->segment, $context->period->getLabel(), $context->plugin);
