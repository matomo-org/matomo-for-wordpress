...
class Controller extends \Piwik\Plugin\Controller
{
    public function siteWithoutData() : string
    {
        ...
        return $this->renderTemplateAs('siteWithoutData', ['backToMatomoLink' => $this->buildAIBotsOverviewUrl($period, $date), 'hideWhatIsNew' => \true], $viewType = 'basic');
    }
    public function getTrackingMethodsForSite() : string
    {
        ...
        foreach ($trackingMethodClasses as $trackingMethodClass) {
            ...
            if (!empty($tabContent) || !empty($link)) {
                ...
                $trackingMethods[] = ['id' => $trackingMethodClass::getId(), 'name' => $trackingMethodClass::getName(), 'content' => $tabContent, 'icon' => $icon ?? '', 'isOthers' => $trackingMethodClass::isOthers(), 'priority' => $trackingMethodClass::getPriority(), 'wasDetected' => $wasDetected, 'link' => $link];
            }
        }
    ...
    public function getEvolutionGraph() : ?string
    {
        ...
        if (Request::fromRequest()->getStringParameter('period', '') !== 'day') {
            $metrics = array_filter($metrics, function ($metric) {
                return !in_array($metric, [\Piwik\Plugins\BotTracking\Metrics::METRIC_AI_CHATBOTS_UNIQUE_DOCUMENT_URLS, \Piwik\Plugins\BotTracking\Metrics::METRIC_AI_CHATBOTS_UNIQUE_PAGE_URLS]);
            });
        }
        ...
        if (empty($view->config->columns_to_display)) {
            $view->config->columns_to_display = [\Piwik\Plugins\BotTracking\Metrics::METRIC_AI_CHATBOTS_REQUESTS];
        }
    ...
    public function noRecentRequestsMessage() : string
    {
        ...
        $period = $request->getStringParameter('period', '');
        $date = $request->getStringParameter('date', '');
        $noDataParams = ['module' => 'BotTracking', 'action' => 'siteWithoutData', 'idSite' => $this->idSite];
        if ($period !== '') {
        ...
        return $this->renderTemplate('noRecentRequestsMessage', ['noDataUrl' => $noDataUrl]);
    }
    public function showNoRecentRequestsMessage() : string
    {
        ...
        $date = $request->getStringParameter('date', '');
        Json::sendHeaderJSON();
        return json_encode(\Piwik\Plugins\BotTracking\NoRecentRequestsMessage::shouldShow($this->idSite, $period, $date));
    }
    /**
    ...
    {
        return Manager::getInstance()->findMultipleComponents('BotTrackingMethod', BotTrackingMethodAbstract::class);
    }
    ...
    protected function buildAIBotsOverviewUrl(string $period, string $date) : string
    {
        ...
        return 'index.php?' . Url::getQueryStringFromParameters($params) . '#?' . Url::getQueryStringFromParameters($hashParams);
    }
}
