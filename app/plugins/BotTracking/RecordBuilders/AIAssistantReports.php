...
class AIAssistantReports extends RecordBuilder
{
    ...
    private function queryAcquiredVisitsByAIAssistant(LogAggregator $logAggregator) : array
    {
        ...
        $sql = sprintf("SELECT `referer_name`, COUNT(*) AS `visits`\n             FROM %s AS `log_visit`\n             WHERE `referer_type` = %d\n               AND `referer_name` <> ''\n               AND %s\n             GROUP BY `referer_name`", Common::prefixTable('log_visit'), Common::REFERRER_TYPE_AI_ASSISTANT, $where);
    ...
    private function populateAssistantTableForActionType(array $tables, int $actionType, LogAggregator $logAggregator, array $visits) : void
    {
        ...
        while ($row = $resultSet->fetch()) {
            ...
            if ($url === RankingQuery::LABEL_SUMMARY_ROW && !$tableRow->isSubtableLoaded()) {
            ...
            $url = $normalized['url'];
            $tableRow->sumRowWithLabelToSubtable($url, [Metrics::COLUMN_REQUESTS => $row['requests']]);
        }
    }
    private function queryBotRequests(LogAggregator $logAggregator, int $actionType)
    {
        ...
        $sql = sprintf("SELECT * FROM (SELECT bot.bot_name, log_action.name AS url, COUNT(*) AS requests\n             FROM `%s` AS bot\n             INNER JOIN `%s` AS log_action ON log_action.idaction = bot.idaction_url\n             WHERE log_action.name IS NOT NULL\n               AND log_action.name <> ''\n               AND log_action.type = %d\n               AND bot.bot_type = ?\n               AND %s\n             GROUP BY bot.bot_name, url WITH ROLLUP) AS rollupQuery\n             ORDER BY requests DESC, bot_name, url", BotRequestsDao::getPrefixedTableName(), Common::prefixTable('log_action'), $actionType, $where);
