...
class NoRecentRequestsMessage
{
    public static function shouldShow(int $idSite, string $period, string $date) : bool
    {
        ...
        $todayLocal = Date::factoryInTimezone('today', $siteTimezone)->setTimezone($siteTimezone);
        ...
        $overlapsLast7 = !$selectedEnd->isEarlier($last7Start) && !$selectedStart->isLater($last7End);
        if (!$overlapsLast7) {
            return \false;
        }
        // server_time is stored in UTC and is indexed with idsite, so querying the latest value is fast.
        $lastRequest = (new BotRequestsDao())->getLastServerTimeForSiteAndBotType($idSite, BotDetector::BOT_TYPE_AI_CHATBOT);
