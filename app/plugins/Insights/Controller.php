...
class Controller extends \Piwik\Plugin\Controller
{
    ...
    private function prepareWidgetView($template)
    {
        ...
        $this->setBasicVariablesView($view);
        $view->properties = array('order_by' => \Piwik\Plugins\Insights\InsightReport::ORDER_BY_ABSOLUTE);
        return $view;
    }
    private function requestApiReport($apiReport)
    {
        ...
        return \Piwik\Plugins\Insights\API::getInstance()->{$apiReport}($this->idSite, $period, $date, $segment);
    }
    private function canGenerateInsights()
    {
        $period = Common::getRequestVar('period', null, 'string');
        $date = Common::getRequestVar('date', null, 'string');
        return \Piwik\Plugins\Insights\API::getInstance()->canGenerateInsights($date, $period);
    }
}
