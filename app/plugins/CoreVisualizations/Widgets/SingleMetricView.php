...
class SingleMetricView extends \Piwik\Widget\Widget
{
    ...
    public function render()
    {
        ...
        if (PluginManager::getInstance()->isPluginActivated('Goals')) {
            $reportMetadata = Request::processRequest('API.getMetadata', ['idSites' => $idSite, 'apiModule' => 'Goals', 'apiAction' => 'get']);
            $reportMetadata = reset($reportMetadata);
            $goalMetrics = array_merge(array_keys($reportMetadata['metrics']), array_keys($reportMetadata['processedMetrics']));
