...
class Evolution extends JqplotDataGenerator
{
    ...
    protected function initChartObjectData($dataTable, $visualization)
    {
        ...
        if ($this->isComparing) {
            ...
            $allSeriesData = $this->collectComparisonSeriesData($rowsToDisplay, $columnsToDisplay, $seriesLabels, $dataTable);
        } else {
            ...
            $seriesState = $this->graph->getForecastSeriesState() ?? $this->collectForecastSeriesState($rowsToDisplay, $columnsToDisplay, $units, $dataTable, \false);
     ...
     */
    protected function getForecastClassifier() : \Piwik\Plugins\CoreVisualizations\JqplotDataGenerator\ForecastMetricClassifier
    {
        if (null === $this->forecastClassifier) {
            $this->forecastClassifier = new \Piwik\Plugins\CoreVisualizations\JqplotDataGenerator\ForecastMetricClassifier();
        }
     ...
     */
    protected function getForecastSubPeriodFetcher() : \Piwik\Plugins\CoreVisualizations\JqplotDataGenerator\ForecastSubPeriodFetcher
    {
        if (null === $this->forecastSubPeriodFetcher) {
            $this->forecastSubPeriodFetcher = new \Piwik\Plugins\CoreVisualizations\JqplotDataGenerator\ForecastSubPeriodFetcher();
        }
    ...
    private function isLinkEnabled()
    {
        ...
        if (!isset($linkEnabled)) {
            ...
            $linkEnabled = !Common::getRequestVar('disableLink', 0, 'int') && Common::getRequestVar('period', 'day') != 'range';
        }
        return $linkEnabled;
    }
    /**
     ...
     */
    private function collectForecastSeriesState(array $rowsToDisplay, array $columnsToDisplay, array $units, DataTable\Map $dataTable, bool $forecastEnabled) : \Piwik\Plugins\CoreVisualizations\JqplotDataGenerator\ForecastSeriesState
    {
        $builder = new \Piwik\Plugins\CoreVisualizations\JqplotDataGenerator\ForecastSeriesStateBuilder();
        foreach ($rowsToDisplay as $rowIdentifier) {
            ...
            foreach ($columnsToDisplay as $columnName) {
                ...
                $builder->addForecastSeries($seriesLabel, $seriesData, $seriesDataAvailability, $columnMonotonicity, $this->getForecastClassifier()->getForecastPrecisionForColumn($columnName, $columnUnit, $columnMonotonicity), $columnName, $rowLabel);
            }
        }
     ...
     */
    private function collectComparisonSeriesData(array $rowsToDisplay, array $columnsToDisplay, array $seriesLabels, DataTable\Map $dataTable) : array
    {
        $allSeriesData = [];
        foreach ($rowsToDisplay as $rowIdentifier) {
            $rowLabel = $this->resolveRowLabel($rowIdentifier);
     ...
     * @param array<int, string> $seriesLabels
     */
    private function setComparisonSeriesData(array &$allSeriesData, array $seriesLabels, $rowLabel, $columnName, DataTable\Map $dataTable) : void
    {
        foreach ($dataTable->getDataTables() as $label => $childTable) {
            ...
            if (empty($row) || empty($row->getComparisons())) {
    ...
    {
        $seriesMetadata = null;
        // maps series labels to any metadata of the series
        $seriesUnits = array();
        // maps series labels to unit labels
        $seriesToXAxis = [];
        // maps series index to x-axis index (groups of metrics for a single comparison will use the same x-axis)
     ...
     */
    private function setDataStates(\Piwik\Plugins\CoreVisualizations\JqplotDataGenerator\Chart $visualization, array $dataTables) : array
    {
    ...
    public function precomputeForecast(DataTable\Map $dataTable) : array
    {
        ...
        $dataStates = $this->computeDataStates($dataTables);
        if (!in_array(ArchiveState::INCOMPLETE, $dataStates, \true)) {
            return [];
        }
        $units = $this->getUnitsForColumnsToDisplay();
        $rowsToDisplay = ($this->properties['rows_to_display'] ?: array_unique($dataTable->getColumn('label'))) ?: [\false];
