...
class HtmlTable extends Visualization
{
    ...
    {
        if (!$this->config->show_totals_row || !$this->dataTable->getRowsCount() || !$this->dataTable->getTotalsRow() || \true !== $this->dataTable->getMetadata(DataTable::TOTALS_ROW_IS_FILTERED_METADATA_NAME)) {
            return;
        }
        $this->assignTemplateVar('isFilteredTotalsRow', \true);
        ...
        $this->config->show_footer_message = empty($this->config->show_footer_message) ? $note : $this->config->show_footer_message . '<br />' . $note;
    }
    /**
    ...
    private function getFilteredTotalsRowContext(?Report $report) : array
    {
        ...
        foreach ($this->config->columns_to_display as $column) {
            if ('label' === $column || !array_key_exists($column, $reportTotals)) {
                continue;
            }
            $context[$column] = array('treatment' => MetricTotalsTreatment::getTreatment($column, $semanticTypes, $processedMetricNames, $aggregationOps), 'reportTotal' => $reportTotals[$column]);
        }
        return $context;
    }
    /**
