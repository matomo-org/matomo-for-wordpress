...
class Cloud extends Visualization
{
    ...
    private $rawValues = [];
    // Raw values stored before metric formatting
    private $formattedValues = [];
    // Formatted values stored after metric formatting
    public static function getDefaultConfig()
    {
        return new \Piwik\Plugins\CoreVisualizations\Visualizations\Cloud\Config();
    }
    ...
    private function generateCloudData()
    {
        ...
        foreach ($this->dataTable->getRows() as $row) {
            ...
            $label = $row->getColumn('label');
            $labelMetadata[$label] = ['logo' => $logo, 'url' => $row->getMetadata('url')];
    ...
    private function getCloudValues()
    {
        ...
        foreach ($this->wordsArray as $word => $popularity) {
            ...
            $return[$word] = ['word' => $word, 'wordTruncated' => $wordTruncated, 'size' => $sizeRange, 'value' => $this->formattedValues[$word], 'percent' => $percent];
        }
        return $return;
    }
    /**
