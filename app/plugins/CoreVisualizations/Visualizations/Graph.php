...
abstract class Graph extends Visualization
{
    ...
    {
        return new \Piwik\Plugins\CoreVisualizations\Visualizations\Graph\Config();
    }
    ...
    public function determineWhichRowsAreSelectable() : void
    {
        ...
        $this->dataTable->filter(function ($dataTable) use($self) {
            ...
            foreach ($dataTable->getRows() as $row) {
                ...
                // ensure we always have the same type
                // build config
                if (!isset($self->selectableRows[$rowIdentifier])) {
                    $self->selectableRows[$rowIdentifier] = ['label' => $rowLabel, 'matcher' => $rowIdentifier, 'displayed' => $self->isRowVisible($rowLabel, $rowIdentifier)];
                }
            }
        });
    }
    ...
    protected function addTranslations() : void
    {
        if ($this->config->add_total_row) {
            ...
            $this->selectableRows['total'] = ['label' => $totalTranslation, 'matcher' => 'total', 'displayed' => $this->isRowVisible($totalTranslation, 'total')];
        }
        if ($this->config->show_goals) {
            $this->config->addTranslations(['nb_conversions' => Piwik::translate('Goals_ColumnConversions'), 'revenue' => Piwik::translate('General_TotalRevenue')]);
        }
        $transformed = [];
        foreach ($this->config->selectable_columns as $column) {
            $transformed[] = ['column' => $column, 'translation' => @$this->config->translations[$column], 'displayed' => in_array($column, $this->config->columns_to_display)];
        }
    ...
    {
        return array('nb_visits', 'nb_actions', 'nb_uniq_visitors', 'nb_users');
    }
