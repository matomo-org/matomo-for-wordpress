...
class API extends \Piwik\Plugin\API
{
    ...
    public function getLocationFromIP($ip = \false, $provider = \false)
    {
        ...
        if (empty($provider)) {
            $provider = \Piwik\Plugins\UserCountry\LocationProvider::getCurrentProviderId();
        }
        $oProvider = \Piwik\Plugins\UserCountry\LocationProvider::getProviderById($provider);
        if (empty($oProvider)) {
    ...
    public function getNumberOfDistinctCountries($idSite, $period, $date, $segment = \false)
    {
        Piwik::checkUserHasViewAccess($idSite);
        $archive = Archive::build($idSite, $period, $date, $segment);
        return $archive->getDataTableFromNumeric(\Piwik\Plugins\UserCountry\Archiver::DISTINCT_COUNTRIES_METRIC);
    }
}
