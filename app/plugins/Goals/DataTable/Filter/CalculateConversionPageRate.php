...
class CalculateConversionPageRate extends BaseFilter
{
    ...
    private function populateRates(DataTable $table, array $goalTotals) : void
    {
        foreach ($table->getRowsWithoutSummaryRow() as $row) {
            ...
            if (is_array($goalsCol)) {
                foreach ($goalsCol as $goalIdString => $metrics) {
                    if (isset($goalsCol[$goalIdString][Metrics::INDEX_GOAL_NB_CONVERSIONS_PAGE_UNIQ]) && isset($goalTotals[$goalIdString])) {
                        $rate = Piwik::getQuotientSafe($goalsCol[$goalIdString][Metrics::INDEX_GOAL_NB_CONVERSIONS_PAGE_UNIQ], $goalTotals[$goalIdString], 3);
