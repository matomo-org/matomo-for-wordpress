...
class GeoIP2AutoUpdater extends Task
{
    ...
    protected function downloadFile($dbType, $url)
    {
        ...
        $zippedOutputPath = self::getTemporaryFolder($zippedFilename, \true);
        ...
        try {
            self::unzipDownloadedFile($zippedOutputPath, $dbType, $url, $unlink = \true);
        } catch (Exception $ex) {
    ...
    public static function unzipDownloadedFile($path, $dbType, $url, $unlink = \false)
    {
        ...
        if (substr($filename, -7, 7) == '.tar.gz') {
            ...
            if (empty($content)) {
                throw new Exception(Piwik::translate('GeoIp2_CannotListContent', array("'{$path}'", $unzip->errorInfo())));
            }
            $fileToExtract = null;
            ...
            if ($fileToExtract === null) {
                throw new Exception(Piwik::translate('GeoIp2_CannotFindGeoIPDatabaseInArchive', array("'{$path}'")));
            }
            // extract JUST the .dat file
            $unzipped = $unzip->extractInString($fileToExtract);
            if (empty($unzipped)) {
                throw new Exception(Piwik::translate('GeoIp2_CannotUnzipGeoIPFile', array("'{$path}'", $unzip->errorInfo())));
            }
        ...
        try {
            ...
            $customDbNames = array('loc' => array(), 'isp' => array());
            ...
            try {
                ...
                Log::info("GeoIP2AutoUpdater: Encountered exception when testing newly downloaded" . " GeoIP 2 database: %s", $e->getMessage());
            ...
            if (@rename($tempFile, $oldDbFile) !== \true) {
    ...
    private static function renameAnyExtraGeolocationDatabases($dbFilename, $dbType)
    {
        ...
        foreach (LocationProviderGeoIp2::$dbNames[$dbType] as $possibleName) {
            ...
            if (file_exists($pathToExistingFile)) {
                ...
                $logger->info("Renaming old geolocation database file {old} to {rename} so new downloaded file {new} will be used.", ['old' => $possibleName, 'rename' => $newFilename, 'new' => $dbFilename]);
                rename($pathToExistingFile, $newFilename);
                // adding timestamp to avoid any potential race conditions
            }
        }
    }
    /**
    ...
    protected static function checkGeoIPUpdateUrl($url)
    {
        ...
        if (\true !== $isValidHost) {
            throw new Exception(Piwik::translate('GeoIp2_InvalidGeoIPUpdateHost', ['<i>' . $url . '</i>', '<i>' . implode(', ', $validHosts) . '</i>', '<i>geolocation_download_from_trusted_hosts</i>']));
        }
    }
    /**
    ...
    {
        if (Option::get(self::LOC_URL_OPTION_NAME) !== \false || Option::get(self::ISP_URL_OPTION_NAME) !== \false) {
            return \true;
        }
        return \false;
    }
    ...
    {
        $instance = new \Piwik\Plugins\GeoIp2\GeoIP2AutoUpdater();
        $instance->update();
    }
    /**
     ...
     */
    protected function performRedundantDbChecks($logErrors = \true)
    {
    ...
    {
        $task = new \Piwik\Plugins\GeoIp2\GeoIP2AutoUpdater();
