...
class API extends \Piwik\Plugin\API
{
    ...
    public function __construct(Service $service, Client $client, InvalidLicenses $expired, PluginManager $pluginManager, \Piwik\Plugins\Marketplace\Environment $environment, PluginTrialService $pluginTrialService)
    {
    ...
    public function createAccount(string $email) : bool
    {
        ...
        try {
            ...
            throw new ValidatorException(Piwik::translate('Marketplace_CreateAccountErrorEmailInvalid', $email));
        }
    ...
    public function saveLicenseKey($licenseKey)
    {
        ...
        } catch (\Piwik\Plugins\Marketplace\Api\Service\Exception $e) {
            if ($e->getCode() === \Piwik\Plugins\Marketplace\Api\Service\Exception::HTTP_ERROR) {
                throw $e;
            }
            $consumer = array();
        }
    ...
    {
        $key = new \Piwik\Plugins\Marketplace\LicenseKey();
        $key->set($licenseKey);
        $this->marketplaceClient->clearAllCacheEntries();
