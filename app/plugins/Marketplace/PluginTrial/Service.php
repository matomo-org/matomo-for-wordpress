...
final class Service
{
    ...
    public function createNotificationsIfNeeded() : void
    {
        ...
        if (!Piwik::hasUserSuperUserAccess()) {
            return;
            // only super users can see and dismiss those notifications
        }
        foreach (\Piwik\Plugins\Marketplace\PluginTrial\Storage::getPluginsInStorage() as $pluginName) {
            $trialRequest = new \Piwik\Plugins\Marketplace\PluginTrial\Notification($pluginName, new \Piwik\Plugins\Marketplace\PluginTrial\Storage($pluginName));
    ...
    public function dismissNotification(string $notificationId) : void
    {
        ...
        if (count($notificationParts) !== 4) {
            return;
            // unable to parse notification id
        }
        $userLogin = $notificationParts[2];
        $pluginName = $notificationParts[3];
        if ($userLogin !== md5(Piwik::getCurrentUserLogin())) {
            return;
            // Don't allow to unset notifications for other users
        }
        $pluginTrial = new \Piwik\Plugins\Marketplace\PluginTrial\Notification($pluginName, new \Piwik\Plugins\Marketplace\PluginTrial\Storage($pluginName));
    ...
    public function cancelRequest(string $pluginName) : void
    {
        ...
        if (Piwik::hasUserSuperUserAccess() && Session::isStarted()) {
            $notification = new \Piwik\Plugins\Marketplace\PluginTrial\Notification($pluginName, new \Piwik\Plugins\Marketplace\PluginTrial\Storage($pluginName));
