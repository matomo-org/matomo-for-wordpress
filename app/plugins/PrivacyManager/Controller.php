...
class Controller extends \Piwik\Plugin\ControllerAdmin
{
    ...
    public function gdprOverview()
    {
        ...
        $purgeDataSettings = \Piwik\Plugins\PrivacyManager\PrivacyManager::getPurgeDataSettings();
        $reportRetention = '';
        ...
        return $this->renderTemplate('gdprOverview', ['reportRetention' => trim($reportRetention), 'rawDataRetention' => trim($rawDataRetention), 'deleteLogsEnable' => $purgeDataSettings['delete_logs_enable'], 'deleteReportsEnable' => $purgeDataSettings['delete_reports_enable'], 'afterGDPROverviewIntroContent' => $afterGDPROverviewIntroContent]);
    }
    public function usersOptOut()
    {
        Piwik::checkUserHasSomeAdminAccess();
        $doNotTrackOptions = [['key' => '1', 'value' => Piwik::translate('PrivacyManager_DoNotTrack_Enable'), 'description' => Piwik::translate('General_Recommended')], ['key' => '0', 'value' => Piwik::translate('PrivacyManager_DoNotTrack_Disable'), 'description' => Piwik::translate('General_NotRecommended')]];
        ...
        foreach ($languages as $language) {
            $languageOptions[] = ['key' => $language['code'], 'value' => $language['name']];
        }
        return $this->renderTemplate('usersOptOut', ['language' => LanguagesManager::getLanguageCodeForCurrentUser(), 'currentLanguageCode' => LanguagesManager::getLanguageCodeForCurrentUser(), 'languageOptions' => $languageOptions, 'doNotTrackOptions' => $doNotTrackOptions, 'dntSupport' => $dntChecker->isActive()]);
    }
    public function consent()
    {
        ...
        if (!empty($consentManager)) {
            ...
            if ($consentManager instanceof ConsentManagerDetectionAbstract) {
                ...
                $view->consentManagerIsConnected = in_array($consentManager::getId(), $this->siteContentDetector->connectedConsentManagers);
            }
        }
    ...
    private function getDeleteDBSizeEstimate($getSettingsFromQuery = false, $forceEstimate = false)
    {
        ...
        $result = ['currentSize' => $formatter->getPrettySizeFromBytes($totalBytes)];
        ...
        if ($doDatabaseSizeEstimate || $forceEstimate == 1) {
            ...
            $deletedDataSummary = \Piwik\Plugins\PrivacyManager\PrivacyManager::getPurgeEstimate($settings);
            ...
            foreach ($tableStatuses as $status) {
                ...
                if (isset($deletedDataSummary[$tableName])) {
                    ...
                    if ($deletedDataSummary[$tableName] === \Piwik\Plugins\PrivacyManager\ReportsPurger::DROP_TABLE) {
    ...
    private function getDeleteDataInfo()
    {
        Piwik::checkUserHasSuperUserAccess();
        $deleteDataInfos = [];
        $deleteDataInfos["config"] = \Piwik\Plugins\PrivacyManager\PrivacyManager::getPurgeDataSettings();
        $deleteDataInfos["deleteTables"] = "<br/>" . implode(", ", \Piwik\Plugins\PrivacyManager\LogDataPurger::getDeleteTableLogTables());
