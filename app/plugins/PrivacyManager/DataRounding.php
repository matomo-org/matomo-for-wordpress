...
class DataRounding
{
    ...
    private const EXCLUDED_SPECIFIC_MAX_METRIC_PATTERN = '/^(max_actions(?:_(?:returning|new|ai_agent|human))?|max_time_(?:network|server|transfer|dom_processing|dom_completion|on_load|generation)|max_bandwidth)$/';
    ...
    public static function roundCountMetricsForRequest(DataTableInterface $dataTable, array $request, ?Report $report = null) : void
    {
        if (!self::requestHasNonEmptySegment($request)) {
            return;
        }
        ...
        self::roundCountMetricsForRequestedSites($dataTable, $requestedSiteIds, $report, null, [self::class, 'isDataRoundingEnabledForSite']);
    }
    /**
     ...
     */
    private static function roundTotalsRowIfPresent(DataTable $table, array $columnsToRound, array $metricTypes, ?Report $report) : void
    {
    ...
    private static function recomputeProcessedMetricForRow(Row $row, string $metricName, ProcessedMetric $processedMetric) : void
    {
     ...
     * @param string[] $columnsToRound
     */
    private static function reconcileTotalsFromRoundedRowsForConstantRowsReport(DataTable $table, array $columnsToRound, ?Report $report) : void
    {
    ...
    private static function collectColumnsToRound(DataTable $table, array $metricTypes) : array
    {
        ...
        foreach ($table->getRows() as $row) {
            foreach ($row->getColumns() as $columnName => $value) {
                ...
                if (!array_key_exists($columnName, $sampleValuesByColumn) || !self::shouldRoundValue($sampleValuesByColumn[$columnName])) {
    ...
    {
        return self::roundArrayValuesRecursive($totals, $metricTypes, \true);
    }
    /**
     * @param array<string, mixed> $values
     * @param array<string, string|null> $metricTypes
    ...
    {
        return self::roundArrayValuesRecursive($values, $metricTypes, \true);
    }
    /**
     * @param array<string, mixed> $values
     * @param array<string, string|null> $metricTypes
     * @return array<string, mixed>
     */
    private static function roundArrayValuesRecursive(array $values, array $metricTypes, bool $allowSemanticTypes, ?string $parentColumnName = null) : array
    {
        foreach ($values as $columnName => $value) {
            ...
            if (self::shouldRoundColumn($metricName, $semanticType) && self::shouldRoundValue($value)) {
    ...
    {
        if ($columnName === '' || in_array($columnName, self::EXCLUDED_COLUMN_NAMES, \true) || self::isIdentifierColumn($columnName) || preg_match(self::CHANGE_COLUMN_PATTERN, $columnName)) {
            return \false;
        }
    ...
    {
        $columnName = strtolower($columnName);
        if (in_array($columnName, self::IDENTIFIER_COLUMN_NAMES, \true)) {
            return \true;
        }
    ...
    {
        try {
            return DataRoundingEnabled::getInstance($idSite)->getValue() === \true;
        } catch (Throwable $e) {
            return \false;
        }
    }
     ...
     * @param int[] $requestedSiteIds
     */
    private static function roundCountMetricsForRequestedSites(DataTableInterface $dataTable, array $requestedSiteIds, ?Report $report, ?int $currentSiteId = null, ?callable $isSiteEnabled = null) : void
    {
        ...
        if ($dataTable instanceof DataTable\Map) {
            foreach ($dataTable->getDataTables() as $key => $childTable) {
                self::roundCountMetricsForRequestedSites($childTable, $requestedSiteIds, $report, self::resolveCurrentSiteIdFromMapKey($dataTable, $key, $currentSiteId), $isSiteEnabled);
            }
            return;
        }
     ...
     * @return array<string, mixed>
     */
    private static function roundArrayValuesForRequestedSites(array $values, array $requestedSiteIds, ?int $currentSiteId = null, ?callable $isSiteEnabled = null) : array
    {
        $isSiteEnabled = $isSiteEnabled ?: [self::class, 'isDataRoundingEnabledForSite'];
        ...
        foreach ($values as $key => $value) {
            ...
            $values[$key] = self::roundArrayValuesForRequestedSites($value, $requestedSiteIds, $currentSiteId, $isSiteEnabled);
        }
        if ($currentSiteId !== null) {
            if (!(bool) call_user_func($isSiteEnabled, $currentSiteId)) {
                return $values;
     ...
     * @param int[] $requestedSiteIds
     */
    private static function roundDataTableForRequestedSites(DataTableInterface $dataTable, array $requestedSiteIds, ?Report $report, ?int $currentSiteId, callable $isSiteEnabled) : void
    {
        if ($dataTable instanceof DataTable\Map) {
        ...
        if (!empty($columnsToRound)) {
            foreach ($dataTable->getRows() as $row) {
                ...
                $shouldRoundRow = $rowSiteId !== null ? (bool) call_user_func($isSiteEnabled, $rowSiteId) : $shouldRoundFallbackRows;
                if (!$shouldRoundRow) {
    ...
    private static function roundDirectScalarArrayValues(array $values) : array
    {
        foreach ($values as $columnName => $value) {
            ...
            if (self::shouldRoundColumn($metricName, null) && self::shouldRoundValue($value)) {
                $values[$columnName] = self::roundToNearestTen((float) $value);
            }
        }
        return $values;
