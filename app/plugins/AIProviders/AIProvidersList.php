...
class AIProvidersList
{
     ...
     */
    public function addProvider(AIProvider $provider, bool $selectable = \true) : bool
    {
        $providerId = $provider->getId();
        if (isset($this->providers[$providerId])) {
            StaticContainer::get(LoggerInterface::class)->warning('AI provider "{id}" is already registered as {existing}; ignoring duplicate registration of {ignored}.', ['id' => $providerId, 'existing' => get_class($this->providers[$providerId]), 'ignored' => get_class($provider)]);
            return \false;
        }
    ...
    {
        return array_values(array_filter($this->providers, function (AIProvider $provider) : bool {
