 ...
 */
class Google extends \Piwik\Plugins\AIProviders\Provider\AIProvider
{
    private const DEFAULT_MODEL = 'gemini-3.1-flash-lite';
    public function __construct()
    {
        parent::__construct('google', 'Google', 'AIProviders_GoogleDefaultModelDescription', \false);
    }
    ...
    public function complete(AIRequest $request, array $configuration) : AIProviderResponse
    {
        ...
        $payload = ['contents' => [['parts' => [['text' => $request->getUserPrompt()]]]], 'generationConfig' => ['maxOutputTokens' => $request->getMaxTokens(), 'temperature' => $request->getTemperature(), 'thinkingConfig' => ['thinkingBudget' => $this->wantsThinking($request) ? $this->thinkingBudget($request) : 0]]];
        ...
        $response = $this->sendJsonRequest($this->getEndpointUrlForModel($model), ['x-goog-api-key' => $this->getApiKey($configuration)], $payload);
        $text = $response['candidates'][0]['content']['parts'][0]['text'] ?? '';
        return $this->buildResponse($request, $model, is_string($text) ? $text : '', isset($response['usageMetadata']['promptTokenCount']) ? (int) $response['usageMetadata']['promptTokenCount'] : null, isset($response['usageMetadata']['candidatesTokenCount']) ? (int) $response['usageMetadata']['candidatesTokenCount'] : null);
    }
    /**
    ...
    {
        $this->sendGetRequest('https://generativelanguage.googleapis.com/v1beta/models', ['x-goog-api-key' => $this->getApiKey($configuration)]);
    }
    ...
    public function converse(AIConversationRequest $request, array $configuration) : AIConversationResponse
    {
        ...
        $payload = ['contents' => $this->canonicalMessagesToGoogle($request->getMessages()), 'generationConfig' => ['maxOutputTokens' => $request->getMaxTokens(), 'temperature' => $request->getTemperature()]];
        ...
        $response = $this->sendJsonRequest($this->getEndpointUrlForModel($model), ['x-goog-api-key' => $this->getApiKey($configuration)], $payload, $request->getTimeoutSeconds());
        $parts = is_array($response['candidates'][0]['content']['parts'] ?? null) ? $response['candidates'][0]['content']['parts'] : [];
        $finishReason = is_string($response['candidates'][0]['finishReason'] ?? null) ? $response['candidates'][0]['finishReason'] : '';
        $content = $this->googlePartsToCanonical($parts);
        $stopReason = $this->resolveStopReason($content, $finishReason);
        return $this->buildConversationResponse($model, $content, $stopReason, isset($response['usageMetadata']['promptTokenCount']) ? (int) $response['usageMetadata']['promptTokenCount'] : null, isset($response['usageMetadata']['candidatesTokenCount']) ? (int) $response['usageMetadata']['candidatesTokenCount'] : null);
    }
    /**
    ...
    private function toolResultBlocksToGoogleParts(array $blocks, array $toolUseNamesById) : array
    {
        ...
        foreach ($blocks as $block) {
            ...
            $name = is_string($toolUseId) && isset($toolUseNamesById[$toolUseId]) ? $toolUseNamesById[$toolUseId] : 'unknown';
            ...
            $parts[] = ['functionResponse' => ['name' => $name, 'response' => $this->toolResultResponseObject($structured, $mcpContent, !empty($block['is_error']))]];
        }
        return $parts;
    }
    /**
    ...
    private function toolCatalogToGoogle(array $tools) : ?array
    {
        ...
        foreach ($tools as $tool) {
            $declarations[] = ['name' => $tool['name'], 'description' => $tool['description'], 'parameters' => $this->googleParameterSchema($tool['inputSchema'])];
        }
    ...
    {
        return $this->stripGoogleUnsupportedKeywords($this->toToolParametersObjectSchema($schema));
    }
    /**
    ...
    {
        unset($schema['additionalProperties'], $schema['$schema'], $schema['$id'], $schema['$ref'], $schema['$defs'], $schema['definitions'], $schema['const'], $schema['patternProperties'], $schema['unevaluatedProperties']);
