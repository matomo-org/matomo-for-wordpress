 ...
 */
class Anthropic extends \Piwik\Plugins\AIProviders\Provider\AIProvider
{
    ...
    {
        parent::__construct('anthropic', 'Anthropic', 'AIProviders_AnthropicDefaultModelDescription', \false);
    }
    ...
    public function complete(AIRequest $request, array $configuration) : AIProviderResponse
    {
        ...
        $payload = ['model' => $model, 'max_tokens' => $request->getMaxTokens(), 'temperature' => $request->getTemperature(), 'messages' => [['role' => 'user', 'content' => $request->getUserPrompt()]]];
        $this->applyThinking($payload, $request);
        $systemPrompt = $this->getSystemPrompt($request);
        if ($systemPrompt !== null && $systemPrompt !== '') {
            $payload['system'] = $systemPrompt;
        }
        $response = $this->sendJsonRequest($this->getEndpointUrl($configuration), ['anthropic-version' => self::ANTHROPIC_VERSION, 'x-api-key' => $this->getApiKey($configuration)], $payload);
        $text = $this->getFirstTextBlock($response['content'] ?? []);
        $stopReason = is_string($response['stop_reason'] ?? null) ? $response['stop_reason'] : null;
        return $this->buildResponse($request, $model, $text, isset($response['usage']['input_tokens']) ? (int) $response['usage']['input_tokens'] : null, isset($response['usage']['output_tokens']) ? (int) $response['usage']['output_tokens'] : null, $stopReason);
    }
    /**
    ...
    {
        $modelsEndpoint = preg_replace('#/messages/?$#', '/models', $this->getEndpointUrl($configuration)) ?? $this->getEndpointUrl($configuration);
        $this->sendGetRequest($modelsEndpoint, ['anthropic-version' => self::ANTHROPIC_VERSION, 'x-api-key' => $this->getApiKey($configuration)]);
    }
    ...
    public function converse(AIConversationRequest $request, array $configuration) : AIConversationResponse
    {
        ...
        $payload = ['model' => $model, 'max_tokens' => $request->getMaxTokens(), 'temperature' => $request->getTemperature(), 'messages' => $this->canonicalMessagesToAnthropic($request->getMessages())];
        ...
        $response = $this->sendJsonRequest($this->getEndpointUrl($configuration), ['anthropic-version' => self::ANTHROPIC_VERSION, 'x-api-key' => $this->getApiKey($configuration)], $payload, $request->getTimeoutSeconds());
        ...
        return $this->buildConversationResponse($model, $this->anthropicContentToCanonical($rawContent), $stopReason, isset($response['usage']['input_tokens']) ? (int) $response['usage']['input_tokens'] : null, isset($response['usage']['output_tokens']) ? (int) $response['usage']['output_tokens'] : null);
    }
    /**
    ...
    private function canonicalBlockToAnthropic(array $block) : ?array
    {
        ...
        if ($type === 'tool_use') {
            ...
            return ['type' => 'tool_use', 'id' => $id, 'name' => $name, 'input' => $input === [] ? new \stdClass() : $input];
        }
        if ($type === 'tool_result') {
            ...
            return ['type' => 'tool_result', 'tool_use_id' => $toolUseId, 'content' => $this->toolResultContentToAnthropic($structured, $mcpContent), 'is_error' => !empty($block['is_error'])];
        }
        return null;
    }
    /**
    ...
    private function toolCatalogToAnthropic(array $tools) : ?array
    {
        ...
        foreach ($tools as $tool) {
            $anthropicTools[] = ['name' => $tool['name'], 'description' => $tool['description'], 'input_schema' => $tool['inputSchema']];
        }
