...
class CustomProvider extends \Piwik\Plugins\AIProviders\Provider\AIProvider
{
    public function __construct()
    {
        parent::__construct('custom-provider', 'Custom Provider', 'AIProviders_CustomProviderDescription', \true);
    }
    /**
    ...
    {
        return $this->completeChatCompletion($request->withModel($this->resolveConfiguredModel($request->getModel(), $configuration)), $this->getChatCompletionsEndpoint($this->getEndpointUrl($configuration)), $this->getBearerAuthorizationHeaders($configuration));
    }
    /**
    ...
    {
        $response = $this->sendGetRequest($this->openAiCompatibleModelsEndpoint($this->getEndpointUrl($configuration)), $this->getBearerAuthorizationHeaders($configuration));
        $models = [];
    ...
    {
        return $this->converseChatCompletion($request->withModel($this->resolveConfiguredModel($request->getModel(), $configuration)), $this->getChatCompletionsEndpoint($this->getEndpointUrl($configuration)), $this->getBearerAuthorizationHeaders($configuration));
    }
    /**
    ...
    {
        $model = $requestModel !== null && $requestModel !== '' ? $requestModel : trim($configuration['model'] ?? '');
        if ($model === '') {
            throw new AIProviderClientException('No model is configured for the custom provider. Select a model in the AI Providers settings.');
        }
        return $model;
    }
    /**
