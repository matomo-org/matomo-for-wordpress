...
class Configuration
{
    ...
    {
        $this->defaultProvider = new SystemSetting(self::SETTING_DEFAULT_PROVIDER, '', FieldConfig::TYPE_STRING, self::PLUGIN_NAME);
        $this->defaultCapabilityLevel = new SystemSetting(self::SETTING_DEFAULT_CAPABILITY_LEVEL, self::CAPABILITY_INSTANT, FieldConfig::TYPE_STRING, self::PLUGIN_NAME);
        $this->providerCredentials = new SystemSetting(self::SETTING_PROVIDER_CREDENTIALS, [], FieldConfig::TYPE_ARRAY, self::PLUGIN_NAME);
    }
    /**
    ...
    public function getSettings(AIProvidersList $providers) : array
    {
        return ['defaultProviderId' => $this->getDefaultProviderId($providers), 'defaultCapabilityLevel' => $this->getDefaultCapabilityLevel(), 'canEditProviderConfiguration' => $this->canEditProviderConfiguration(), 'canEditCapabilityLevel' => $this->canEditCapabilityLevel(), 'capabilityLevels' => $this->getCapabilityLevels(), 'providers' => array_map(function (AIProvider $provider) : array {
            ...
            return array_merge($provider->toArray(), ['configuration' => ['hasApiKey' => !empty($providerConfiguration['apiKey']), 'endpointUrl' => $providerConfiguration['endpointUrl'], 'model' => $providerConfiguration['model'], 'useFipsEndpoint' => $providerConfiguration['useFipsEndpoint'], 'isUsable' => $provider->isConfigured($providerConfiguration)]]);
...
string $providerConfigurationsJson) : void
    ...
    public function getProviderConfiguration(AIProvider $provider) : array
    {
        ...
        $storedConfiguration = $this->getProviderConfigurations()[$providerId] ?? ['apiKey' => '', 'endpointUrl' => '', 'model' => '', 'useFipsEndpoint' => \false];
        $configFileConfiguration = $this->getConfigFileProviderConfiguration($providerId);
        return ['apiKey' => $configFileConfiguration['apiKey'] !== '' ? $configFileConfiguration['apiKey'] : $storedConfiguration['apiKey'], 'endpointUrl' => $this->resolveEndpointUrl($provider, $configFileConfiguration, $storedConfiguration), 'model' => $configFileConfiguration['model'] !== '' ? $configFileConfiguration['model'] : $storedConfiguration['model'], 'useFipsEndpoint' => $configFileConfiguration['useFipsEndpoint'] !== null ? $configFileConfiguration['useFipsEndpoint'] : $storedConfiguration['useFipsEndpoint']];
    }
    /**
...
array $storedConfiguration) : string
    ...
    {
        return $this->getConfigFileProviderConfiguration($provider->getId())['apiKey'] !== '' && $provider->supportsCustomEndpoint() && $provider->endpointFieldRequiresUrl();
    }
    /**
...
array $submittedProviderConfiguration = []) : array
    {
        ...
        $this->checkSubmittedEndpointUrlIsAllowed($provider, $submittedProviderConfiguration, $this->getSubmittedEndpointUrl($submittedProviderConfiguration, $provider));
        ...
        $submittedProviderConfiguration = array_merge($existingConfiguration, $submittedProviderConfiguration);
        return ['apiKey' => $this->getSubmittedApiKey($submittedProviderConfiguration, [$providerId => $existingConfiguration], $providerId), 'endpointUrl' => $this->getSubmittedEndpointUrl($submittedProviderConfiguration, $provider), 'model' => $this->getSubmittedModel($submittedProviderConfiguration, $provider), 'useFipsEndpoint' => $this->getSubmittedUseFipsEndpoint($submittedProviderConfiguration, $provider)];
    }
    /**
    ...
    {
        return [self::CAPABILITY_INSTANT => ['label' => 'AIProviders_InstantCapability', 'description' => 'AIProviders_InstantCapabilityDescription'], self::CAPABILITY_THINKING => ['label' => 'AIProviders_ThinkingCapability', 'description' => 'AIProviders_ThinkingCapabilityDescription']];
    }
    /**
    ...
    public function getDefaultProviderId(AIProvidersList $providers) : string
    {
        ...
        if (is_string($providerId) && $providers->isSelectable($providerId) && $this->isProviderUsable($providers->getProvider($providerId))) {
            return $providerId;
        }
        if ($providers->isSelectable(self::DEFAULT_PROVIDER_ID) && $this->isProviderUsable($providers->getProvider(self::DEFAULT_PROVIDER_ID))) {
    ...
    private function saveDefaultProviderId(AIProvidersList $providers, string $providerId) : void
    {
        ...
        if ($provider === null || !$providers->isSelectable($providerId)) {
            throw new InvalidArgumentException(Piwik::translate('AIProviders_ErrorUnknownProvider', $providerId));
        }
        if (!$this->isProviderUsable($provider)) {
            throw new InvalidArgumentException(Piwik::translate('AIProviders_ErrorProviderNotConfigured', $provider->getName()));
        }
    ...
    private function saveDefaultCapabilityLevel(string $capabilityLevel) : void
    {
        ...
        if (!array_key_exists($capabilityLevel, $this->getCapabilityLevels())) {
            throw new InvalidArgumentException(Piwik::translate('AIProviders_ErrorUnknownCapabilityLevel', $capabilityLevel));
        }
...
array $submittedProviderConfigurations) : void
    {
        ...
        foreach ($providers->getSelectableProviders() as $provider) {
            ...
            $providerConfigurations[$providerId] = ['apiKey' => $apiKey, 'endpointUrl' => $endpointUrl, 'model' => $model, 'useFipsEndpoint' => $useFipsEndpoint];
        }
        $this->providerCredentials->setValue($providerConfigurations);
        $this->providerCredentials->save();
    }
...
array $submittedProviderConfiguration, string $endpointUrl) : void
    {
        ...
        $messageKey = $this->getConfigFileProviderConfiguration($provider->getId())['endpointUrl'] === '' ? 'AIProviders_ErrorEndpointUrlManagedWithApiKey' : 'AIProviders_ErrorEndpointManaged';
        throw new InvalidArgumentException(Piwik::translate($messageKey, [$provider->getName(), $provider->getId() . 'EndpointUrl']));
    }
    /**
...
array $existingProviderConfigurations) : string
    ...
    private function isEndpointUrlSuppliedCentrally(AIProvider $provider) : bool
    {
        ...
        return $this->getConfigFileProviderConfiguration($provider->getId())['endpointUrl'] !== '' || $this->isEndpointUrlPinnedBySuppliedApiKey($provider);
    }
    /**
...
array $submittedProviderConfiguration) : bool
    ...
    private function getSubmittedEndpointUrl(array $submittedProviderConfiguration, AIProvider $provider) : string
    {
        ...
        $endpointUrl = isset($submittedProviderConfiguration['endpointUrl']) && is_string($submittedProviderConfiguration['endpointUrl']) ? trim($submittedProviderConfiguration['endpointUrl']) : '';
        if ($endpointUrl === '') {
            return '';
        }
        try {
     ...
     */
    private function invalidEndpointFieldException(AIProvider $provider, ?AIProviderClientException $cause = null) : InvalidArgumentException
    {
        return new InvalidArgumentException(Piwik::translate($provider->getEndpointFieldErrorMessage(), $provider->getName()), 0, $cause);
    }
    /**
     * @param array<string, mixed> $submittedProviderConfiguration
     */
    ...
    {
        return $provider->supportsFipsEndpoint() && !empty($submittedProviderConfiguration['useFipsEndpoint']);
    }
    /**
    ...
    private function getSubmittedModel(array $submittedProviderConfiguration, AIProvider $provider) : string
    {
        ...
        return isset($submittedProviderConfiguration['model']) && is_string($submittedProviderConfiguration['model']) ? trim($submittedProviderConfiguration['model']) : '';
    }
    ...
    public function isPluginAllowedToSelectProvider(string $pluginName) : bool
    {
        ...
        return in_array($pluginName, $this->getProviderSelectionAllowlist(), \true);
    }
    /**
     * @return string[]
     */
    ...
    {
        return ['apiKey' => $this->getConfigFileValue($providerId, 'ApiKey', 'API_KEY'), 'endpointUrl' => $this->getConfigFileValue($providerId, 'EndpointUrl', 'ENDPOINT_URL'), 'model' => $this->getConfigFileValue($providerId, 'Model', 'MODEL'), 'useFipsEndpoint' => $this->getConfigFileBooleanValue($providerId, 'UseFipsEndpoint', 'USE_FIPS_ENDPOINT')];
    }
