...
class API extends PluginAPI
{
    ...
    public function __construct(Configuration $configuration, \Piwik\Plugins\AIProviders\AIProviderService $aiProviderService)
    {
...
string $providerConfigurations = '{}') : array
    {
        ...
        $configuration->saveSettings($providers, $defaultProviderId, $defaultCapabilityLevel, $providerConfigurations);
...
string $providerConfiguration = '{}') : array
    {
        ...
        $configuration = $this->configuration->getProviderConfigurationForUse($provider, $this->decodeProviderConfiguration($providerConfiguration));
        $this->aiProviderService->testProviderConnection($provider, $configuration);
        return ['providerId' => $provider->getId(), 'providerName' => $provider->getName(), 'models' => $provider->listModels($configuration)];
    }
    /**
     ...
     * operate on them.
     */
    private function getSelectableProvider(\Piwik\Plugins\AIProviders\AIProvidersList $providers, string $providerId) : AIProvider
    {
