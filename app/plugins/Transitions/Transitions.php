...
class Transitions extends \Piwik\Plugin
{
    ...
    {
        return array('AssetManager.getStylesheetFiles' => 'getStylesheetFiles', 'AssetManager.getJavaScriptFiles' => 'getJsFiles', 'Translate.getClientSideTranslationKeys' => 'getClientSideTranslationKeys', 'API.getPagesComparisonsDisabledFor' => 'getPagesComparisonsDisabledFor', 'Template.jsGlobalVariables' => 'addJsGlobalVariables');
    }
    ...
    public static function getPeriodAllowedConfig($idSite) : string
    {
        ...
        if (!$generalMaxPeriodAllowed && !$siteMaxPeriodAllowed) {
            return 'all';
            // No config setting, so all periods are valid
        }
