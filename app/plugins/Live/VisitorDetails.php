...
class VisitorDetails extends \Piwik\Plugins\Live\VisitorDetailsAbstract
{
    ...
    {
        $response = Request::processRequest('Live.getFirstVisitForVisitorId', ['idSite' => $this->getIdSite(), 'visitorId' => $this->getVisitorId()]);
    ...
    private function getVisitorProfileVisitSummary($visit, Site $site) : array
    {
        ...
        return ['date' => $firstActionTimestamp, 'prettyDate' => $firstActionDate->getLocalized(Date::DATE_FORMAT_LONG), 'daysAgo' => (int) Date::secondsToDays($today->getTimestamp() - $firstActionDate->getTimestamp()), 'referrerType' => $visit->getColumn('referrerType'), 'referrerUrl' => $visit->getColumn('referrerUrl') ?: '', 'referralSummary' => self::getReferrerSummaryForVisit($visit)];
    }
    /**
