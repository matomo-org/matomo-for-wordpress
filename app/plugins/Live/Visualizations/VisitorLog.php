...
class VisitorLog extends Visualization
{
    ...
    {
        $this->requestConfig->addPropertiesThatShouldBeAvailableClientSide(array('filter_limit', 'filter_offset', 'filter_sort_column', 'filter_sort_order'));
        if (!is_numeric($this->requestConfig->filter_limit) || $this->requestConfig->filter_limit == -1) {
    ...
    public function beforeRender()
    {
        ...
        if (!$this->isInPopover()) {
            $this->config->footer_icons = array(array('class' => 'tableAllColumnsSwitch', 'buttons' => array(array('id' => static::ID, 'title' => Piwik::translate('Live_LinkVisitorLog'), 'icon' => 'plugins/Morpheus/images/table.png'))));
        } else {
    ...
    public static function groupActionsByPageviewId(DataTable $table)
    {
        foreach ($table->getRows() as $row) {
            ...
            foreach ($row->getColumn('actionDetails') as $key => $action) {
                ...
                if (empty($action['idpageview']) && self::isPageviewAction($action)) {
                    $actionGroups[] = ['pageviewAction' => null, 'actionsOnPage' => [$action], 'refreshActions' => []];
                    continue;
                }
                ...
                if (empty($actionGroups[$idPageView])) {
                    $actionGroups[$idPageView] = ['pageviewAction' => null, 'actionsOnPage' => [], 'refreshActions' => []];
                }
    ...
    private static function mergeRefreshes(array $actionGroups)
    {
        ...
        foreach ($actionGroups as $idPageview => $group) {
            ...
            if ($isLastGroupEmpty && $isPageviewActionSame) {
    ...
    {
        return $action['type'] != 'action' && $action['type'] != Action::TYPE_PAGE_URL && $action['type'] != Action::TYPE_PAGE_TITLE;
    }
}
