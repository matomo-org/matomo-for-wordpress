...
class PolicyManager
{
    ...
    private static $settingTypesMap = [self::SETTING_TYPE_SYSTEM => ['trait' => SystemGetterTrait::class, 'method' => 'getSystemSettingShortName'], self::SETTING_TYPE_MEASURABLE => ['trait' => MeasurableGetterTrait::class, 'method' => 'getMeasurableSettingShortName'], self::SETTING_TYPE_CUSTOM => ['trait' => CustomGetterTrait::class, 'method' => 'getCustomSettingShortName'], self::SETTING_TYPE_OPTION => ['trait' => OptionGetterTrait::class, 'method' => 'getOptionSettingShortName'], self::SETTING_TYPE_CONFIG => ['trait' => ConfigGetterTrait::class, 'method' => 'getConfigSettingShortName']];
    /**
     * @return array<class-string<CompliancePolicy>>
     */
    public static function getAllPolicies() : array
    {
        return [\Piwik\Policy\CnilPolicy::class];
    }
    /**
    ...
    protected static function getAllSettings(?string $settingType = null) : array
    {
        ...
        foreach ($settings as $setting) {
            if (!is_a($setting, PolicyComparisonInterface::class, \true)) {
                continue;
            }
            if ($settingType && !in_array(self::$settingTypesMap[$settingType]['trait'], class_uses($setting), \true)) {
                continue;
            }
            $underPolicy[] = $setting;
        }
    ...
    {
        if (!is_a($policyClass, \Piwik\Policy\CompliancePolicy::class, \true)) {
    ...
    {
        $map = [MeasurableSetting::class => self::SETTING_TYPE_MEASURABLE, MeasurableProperty::class => self::SETTING_TYPE_MEASURABLE, SystemSetting::class => self::SETTING_TYPE_SYSTEM, SystemConfigSetting::class => self::SETTING_TYPE_CONFIG];
    ...
    {
        if (!$settingType || !in_array($settingType, array_keys(self::$settingTypesMap), \true)) {
            return [];
        }
        $policies = static::getAllPolicies();
        $settings = [];
        foreach ($policies as $policyClass) {
            ...
            foreach ($controlledSettings as $controlledSetting) {
                if ($settingName === call_user_func([$controlledSetting, self::$settingTypesMap[$settingType]['method']])) {
                    $settings[$policyClass::getName()] = ['requiredValue' => $controlledSetting::getPolicyRequirements()[$policyClass]];
                }
            }
        }
        return $settings;
