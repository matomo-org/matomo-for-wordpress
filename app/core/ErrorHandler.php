...
class ErrorHandler
{
    ...
    public static function errorHandler($errno, $errstr, $errfile, $errline)
    {
        ...
        switch ($errno) {
            ...
            case \E_USER_ERROR:
                ...
                $e = new \Exception();
                $backtrace = \Piwik\ExceptionHandler::shouldPrintBackTraceWithMessage() ? $e->getTraceAsString() : '';
    ...
    {
        return sprintf("%s(%d): %s - %s - Matomo " . (class_exists('Piwik\\Version') ? \Piwik\Version::VERSION : '') . " - Please report this message in the Matomo forums: https://forum.matomo.org (please do a search first as it might have been reported already)", $errfile, $errline, \Piwik\ErrorHandler::getErrNoString($errno), $errstr);
    }
    private static function getHtmlMessage($errno, $errstr, $errfile, $errline, $trace)
    {
        $trace = \Piwik\Log::$debugBacktraceForTests ?: $trace;
        $message = \Piwik\ErrorHandler::getErrNoString($errno) . ' - ' . $errstr;
        $html = "<p>There is an error. Please report the message (Matomo " . (class_exists('Piwik\\Version') ? \Piwik\Version::VERSION : '') . ")\n        and full backtrace in the <a target='_blank' rel='noreferrer noopener' href='https://forum.matomo.org'>Matomo forums</a> (please do a search first as it might have been reported already!).</p>";
