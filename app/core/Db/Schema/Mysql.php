...
class Mysql implements SchemaInterface
{
    ...
    public function hasReachedEOL() : bool
    {
        ...
        if (version_compare($currentVersion, '8.0', '>=') && version_compare($currentVersion, '8.1', '<') && Date::today()->isEarlier(Date::factory('2026-05-01'))) {
            return \false;
        }
        // Support for 8.4 LTS ends in April 2032
        if (version_compare($currentVersion, '8.4', '>=') && version_compare($currentVersion, '8.5', '<') && Date::today()->isEarlier(Date::factory('2032-05-01'))) {
            return \false;
        }
