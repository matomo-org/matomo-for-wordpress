...
class ArchiveFilter
{
    ...
    public function filterArchive($archive)
    {
        ...
        if ($this->disableSegmentsArchiving && !empty($segment)) {
        ...
        if (!empty($this->restrictToDateRange) && ($this->restrictToDateRange[0]->isLater(Date::factory($archive['date2'])) || $this->restrictToDateRange[1]->isEarlier(Date::factory($archive['date1'])))) {
        ...
        if (!empty($this->restrictToPeriods) && !in_array($periodLabel, $this->restrictToPeriods)) {
    ...
    public function setRestrictToDateRange($restrictToDateRange)
    {
        ...
        try {
            ...
            $parts = [Date::factory($parts[0]), Date::factory($parts[1])];
        } catch (\Exception $ex) {
