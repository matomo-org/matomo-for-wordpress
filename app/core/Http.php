...
class Http
{
     ...
     * @api
     */
    public static function sendHttpRequest($aUrl, $timeout, $userAgent = null, $destinationPath = null, $followDepth = 0, $acceptLanguage = \false, $byteRange = \false, $getExtendedInfo = \false, $httpMethod = 'GET', $httpUsername = null, $httpPassword = null, $checkHostIsAllowed = \true, $validateEgressIp = \false)
    {
        // create output file
        ...
        return self::sendHttpRequestBy($transport, $aUrl, $timeout, $userAgent, $destinationPath, $file, $followDepth ?? 0, $acceptLanguage, $acceptInvalidSslCertificate = \false, $byteRange, $getExtendedInfo, $httpMethod, $httpUsername, $httpPassword, null, [], null, $checkHostIsAllowed, $validateEgressIp);
    }
    /**
    ...
    private static function assertHostNotBlocked(?string $host) : void
    {
        ...
        foreach ($disallowedHosts as $disallowedHost) {
            if (preg_match(self::convertWildcardToPattern($disallowedHost), $host) === 1) {
                throw new Exception(sprintf('Hostname %s is in list of disallowed hosts', $host));
            }
        }
    }
    ...
    public static function sendHttpRequestBy($method, $aUrl, $timeout, $userAgent = null, $destinationPath = null, $file = null, $followDepth = 0, $acceptLanguage = \false, $acceptInvalidSslCertificate = \false, $byteRange = \false, $getExtendedInfo = \false, $httpMethod = 'GET', $httpUsername = null, $httpPassword = null, $requestBody = null, $additionalHeaders = array(), $forcePost = null, $checkHostIsAllowed = \true, $validateEgressIp = \false)
    {
        ...
        if (!$isAllowed) {
            throw new Exception(sprintf('Protocol %s not in list of allowed protocols: %s', $parsedUrl['scheme'], $allowedProtocols));
        }
        ...
        if ($validateEgressIp) {
            ...
            [$canonicalHost, $pinnedIp] = StaticContainer::get(EgressHostValidator::class)->resolveTarget((string) ($parsedUrl['host'] ?? ''));
        ...
        $via = 'Via: ' . (isset($_SERVER['HTTP_VIA']) && !empty($_SERVER['HTTP_VIA']) ? $_SERVER['HTTP_VIA'] . ', ' : '') . \Piwik\Version::VERSION . ' ' . ($userAgent ? " ({$userAgent})" : '');
        // range header
        $rangeBytes = '';
        $rangeHeader = '';
        if (!empty($byteRange)) {
        ...
        $httpEventParams = array('httpMethod' => $httpMethod, 'body' => $requestBody, 'userAgent' => $userAgent, 'timeout' => $timeout, 'headers' => array_map('trim', array_filter(array_merge([$rangeHeader, $via, $httpAuth, $acceptLanguage], $additionalHeaders))), 'verifySsl' => !$acceptInvalidSslCertificate, 'destinationPath' => $destinationPath, 'validateEgressIp' => $validateEgressIp);
        /**
        ...
        if ($method == 'socket') {
            ...
            if ($validateEgressIp && $status >= 300 && $status < 400 && $status !== 304) {
                ...
                if ($redirectUrl !== '') {
                    ...
                    return self::sendHttpRequestBy($method, $redirectUrl, $remainingTimeout, $userAgent, $destinationPath, $file, $followDepth + 1, $acceptLanguage, $acceptInvalidSslCertificate, $byteRange, $getExtendedInfo, $httpMethod, $httpUsername, $httpPassword, $requestBody, $additionalHeaders, $forcePost, $checkHostIsAllowed, $validateEgressIp);
                }
            }
        } else {
    ...
    public static function downloadChunk($url, $outputPath, $isContinuation)
    {
        ...
        if (!$isContinuation && file_exists($outputPath)) {
            throw new Exception(\Piwik\Piwik::translate('General_DownloadFail_FileExists', "'" . $outputPath . "'") . ' ' . \Piwik\Piwik::translate('General_DownloadPleaseRemoveExisting'));
        }
    ...
    {
        return !empty($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : 'Matomo/' . \Piwik\Version::VERSION;
    }
    public static function getClientHintsFromServerVariables() : array
    {
        $clientHints = [];
        foreach ($_SERVER as $key => $value) {
            if (0 === strpos(strtolower($key), strtolower('HTTP_SEC_CH_UA')) || 'X_HTTP_REQUESTED_WITH' === strtoupper($key)) {
    ...
    private static function getProxyConfiguration($url) : array
    {
        ...
        $proxyPort = \Piwik\Config::getInstance()->proxy['port'];
        $proxyUser = \Piwik\Config::getInstance()->proxy['username'];
        $proxyPassword = \Piwik\Config::getInstance()->proxy['password'];
        $proxyExclude = \Piwik\Config::getInstance()->proxy['exclude'];
    ...
    public static function isUpdatingOverHttps()
    {
        ...
        $usingMethodSupportingHttps = \Piwik\Http::getTransportMethod() !== 'socket';
