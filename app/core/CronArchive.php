...
class CronArchive
{
    ...
    private $requests = 0;
    private $archiveAndRespectTTL = \true;
    ...
    public function __construct(?LoggerInterface $logger = null)
    {
        ...
        $this->isArchiveProfilingEnabled = \Piwik\Config::getInstance()->Debug['archiving_profile'] == 1;
        $this->model = StaticContainer::get(Model::class);
        $this->periodIdsToLabels = array_flip(\Piwik\Piwik::$idPeriods);
    ...
    {
        return \Piwik\Config::getInstance()->General['maintenance_mode'] == 1;
    }
    /**
    ...
    public function main()
    {
        ...
        $self = $this;
        \Piwik\Access::doAsSuperUser(function () use($self) {
            try {
                $self->init();
                $self->run();
    ...
    public function init()
    {
         ...
         */
        \Piwik\Piwik::postEvent('CronArchive.init.start', [$this]);
        ...
        \Piwik\Option::set(self::OPTION_ARCHIVING_STARTED_TS, time());
        ...
        if (method_exists($this->websiteIdArchiveList, 'isContinuingPreviousRun') && $this->websiteIdArchiveList->isContinuingPreviousRun()) {
         ...
         */
        \Piwik\Piwik::postEvent('CronArchive.init.finish', [$this->allWebsites]);
    }
    /**
    ...
    public function run()
    {
        $pid = \Piwik\Common::getProcessId();
        $timer = new \Piwik\Timer();
        ...
        if ($failedJobs) {
            $this->logger->info("Found {failed} failed jobs (ts_invalidated older than 1 day), resetting status to try them again.", ['failed' => $failedJobs]);
        }
        $countOfProcesses = $this->getMaxConcurrentApiRequests();
        $queueConsumer = new QueueConsumer($this->logger, $this->websiteIdArchiveList, $countOfProcesses, $pid, $this->model, $this->segmentArchiving, $this, $this->cliMultiRequestParser, $this->archiveFilter);
        ...
        $this->logger->info("done: " . $this->requests . " req, " . round($timer->getTimeMs()) . " ms, " . (empty($this->errors) ? self::NO_ERROR : count($this->errors) . " errors."));
    ...
    private function launchArchivingFor($archives, QueueConsumer $queueConsumer)
    {
        ...
        foreach ($archives as $index => $archive) {
            ...
            $params = new Parameters($site, $period, new \Piwik\Segment($segment, [$idSite], $period->getDateTimeStart()->setTimezone($site->getTimezone()), $period->getDateTimeEnd()->setTimezone($site->getTimezone())));
            if (!empty($plugin)) {
            ...
            if ($loader->canSkipThisArchive()) {
                $this->logger->info("Found no visits for site ID = {idSite}, {period} ({date1},{date2}), site is using the tracker so skipping archiving...", ['idSite' => $idSite, 'period' => $this->periodIdsToLabels[$archive['period']], 'date1' => $archive['date1'], 'date2' => $archive['date2']]);
        ...
        if (empty($urls)) {
            return 0;
            // all URLs had no visits and were using the tracker
        }
        ...
        foreach ($urls as $index => $url) {
            ...
            $stats = json_decode($content, $assoc = \true);
            if (!is_array($stats)) {
                $this->logger->info(var_export($content, \true));
    ...
    private function generateUrlToArchiveFromArchiveInfo($archive)
    {
        ...
        if (!empty($segment)) {
            $shouldSkipToday = $this->archiveFilter->isSkipSegmentsForToday() && !$this->wasSegmentChangedRecently($segment, $this->segmentArchiving->getAllSegments());
            if ($shouldSkipToday) {
    ...
    public function end()
    {
         ...
         * @param CronArchive $this
         */
        \Piwik\Piwik::postEvent('CronArchive.end', [$this]);
        if (empty($this->errors)) {
            // No error -> Logs the successful script execution until completion
            \Piwik\Option::set(self::OPTION_ARCHIVING_FINISHED_TS, time());
            return;
        }
    ...
    public function runScheduledTasks()
    {
        ...
        $this->disconnectDb();
        \Piwik\Piwik::addAction('ScheduledTasks.execute.end', function () {
        ...
        Rules::$disablePureOutdatedArchive = \true;
    ...
    private function disconnectDb()
    {
        ...
        if (time() > $this->lastDbReset + $twoHoursInSeconds) {
            ...
            \Piwik\Db::destroyDatabaseObject();
            \Piwik\Tracker::disconnectCachedDbConnection();
        }
    }
    /**
    ...
    public function logError($m)
    {
        if (!defined('PIWIK_ARCHIVE_NO_TRUNCATE')) {
            ...
            if (mb_strlen($m) > self::TRUNCATE_ERROR_MESSAGE_SUMMARY) {
                ...
                $m = mb_substr($m, 0, self::TRUNCATE_ERROR_MESSAGE_SUMMARY - $numCharactersKeepFromEnd) . ' ... ' . mb_substr($m, -1 * $numCharactersKeepFromEnd);
            }
        }
        $this->errors[] = $m;
        $this->logger->error($m);
    ...
    {
        if (empty($response) || stripos($response, 'error') !== \false) {
    ...
    public function filterWebsiteIds(&$websiteIds, $allWebsites)
    {
         ...
         */
        \Piwik\Piwik::postEvent('CronArchive.filterWebsiteIds', [&$websiteIds]);
    }
    /**
     * @internal
     * @param $api
    ...
    public function invalidateRecentDate(string $dateStr, int $idSite) : void
    {
        ...
        $date = \Piwik\Date::factoryInTimezone($dateStr, $timezone);
        ...
        $this->logger->info("  Will invalidate archived reports for {$dateStr} in site ID = {idSite}'s timezone ({date}).", ['idSite' => $idSite, 'date' => $date->getDatetime()]);
        ...
        $this->invalidateWithSegments([$idSite], $date->toString(), 'day', $isYesterday, $onlyProcessSegmentsChangedRecently);
    }
    private function invalidateWithSegments($idSites, $date, string $period, bool $skipWhenRunningOrNewEnoughArchiveExists = \false, bool $onlyProcessSegmentsChangedRecently = \false)
    {
        if ($date instanceof \Piwik\Date) {
            $date = $date->toString();
        }
        $periodObj = PeriodFactory::build($period, $date);
        if ($period == 'range') {
            $date = [$date];
            // so we don't split on the ',' in invalidateArchivedReports
        }
        if (!is_array($idSites)) {
            $idSites = [$idSites];
        }
        foreach ($idSites as $idSite) {
            ...
            foreach ($this->segmentArchiving->getAllSegmentsToArchive($idSite) as $segmentDefinition) {
                ...
                $segmentObj = new \Piwik\Segment($segmentDefinition, [$idSite], $periodObj->getDateTimeStart()->setTimezone($site->getTimezone()), $periodObj->getDateTimeEnd()->setTimezone($site->getTimezone()));
                $params = new Parameters($site, $periodObj, $segmentObj);
                if ($this->canWeSkipInvalidatingBecauseThereIsAUsablePeriod($params, $skipWhenRunningOrNewEnoughArchiveExists)) {
                    $this->logger->debug('  Found usable archive for {archive}, skipping invalidation.', ['archive' => $params]);
                    ...
                    $this->getApiToInvalidateArchivedReport()->invalidateArchivedReports($idSite, $date, $period, $segmentDefinition, $cascadeDown = \false, $period === 'range');
                }
            }
        }
    }
    ...
    {
        try {
            new \Piwik\Segment($segmentDefinition, $idSites);
        } catch (\Exception $e) {
    ...
    private function canWeSkipInvalidatingBecauseInvalidationAlreadyInProgress(int $idSite, \Piwik\Period $period, ?\Piwik\Segment $segment = null) : bool
    {
        $invalidationsInProgress = $this->model->getInvalidationsInProgress($idSite);
        $timezone = \Piwik\Site::getTimezoneFor($idSite);
        $doneFlag = Rules::getDoneFlagArchiveContainsAllPlugins($segment ?? new \Piwik\Segment('', [$idSite]));
        $endOfDayInTimezone = $period->getDateEnd()->getEndOfDay();
        foreach ($invalidationsInProgress as $invalidation) {
            if ($invalidation['name'] === $doneFlag && $invalidation['period'] == $period->getId() && $period->getDateStart()->toString() === $invalidation['date1'] && \Piwik\Date::factory($invalidation['ts_started'], $timezone)->isLater($endOfDayInTimezone)) {
                return \true;
            }
        }
        return \false;
    ...
    {
        $timezone = \Piwik\Site::getTimezoneFor($params->getSite()->getId());
        $today = \Piwik\Date::factoryInTimezone('today', $timezone);
        $yesterday = \Piwik\Date::factoryInTimezone('yesterday', $timezone);
        ...
        $minArchiveProcessedTime = $doNotIncludeTtlInExistingArchiveCheck ? null : \Piwik\Date::now()->subSeconds(Rules::getPeriodArchiveTimeToLiveDefault($params->getPeriod()->getLabel()));
        ...
        if ($isYesterday && !empty($idArchive) && \Piwik\Date::factory($tsArchived, $timezone)->toString() !== $today->toString()) {
            return \false;
        }
    ...
    {
        $table = \Piwik\Common::prefixTable('archive_invalidations');
        $bind = [$archiveToProcess['idsite'], $archiveToProcess['name'], $archiveToProcess['period'], $archiveToProcess['date1'], $archiveToProcess['date2']];
        $reportClause = '';
        ...
        $higherPeriods = \Piwik\Db::fetchAll($sql, $bind);
        $higherPeriods = array_column($higherPeriods, 'period');
        $invalidationsToInsert = [];
        foreach (\Piwik\Piwik::$idPeriods as $label => $id) {
            // lower period than the one we're processing or range, don't care
            if ($id <= $archiveToProcess['period'] || $label == 'range') {
                continue;
            }
            if (in_array($id, $higherPeriods)) {
                // period exists in table
                continue;
            }
            // period is disabled in API
            if (!PeriodFactory::isPeriodEnabledForAPI($label) || PeriodFactory::isAnyLowerPeriodDisabledForAPI($label)) {
                continue;
            }
            // archive is for a week that is over two months, we don't need to care about the month
            if ($label == 'month' && \Piwik\Date::factory($archiveToProcess['date1'])->toString('m') != \Piwik\Date::factory($archiveToProcess['date2'])->toString('m')) {
                continue;
            }
            // archive is for a week that is over two years, we don't need to care about the year
            if ($label == 'year' && \Piwik\Date::factory($archiveToProcess['date1'])->toString('y') != \Piwik\Date::factory($archiveToProcess['date2'])->toString('y')) {
                continue;
            }
            $period = \Piwik\Period\Factory::build($label, $archiveToProcess['date1']);
            $invalidationToInsert = ['idarchive' => null, 'name' => $archiveToProcess['name'], 'report' => $archiveToProcess['report'], 'idsite' => $archiveToProcess['idsite'], 'date1' => $period->getDateStart()->getDatetime(), 'date2' => $period->getDateEnd()->getDatetime(), 'period' => $id, 'ts_invalidated' => $archiveToProcess['ts_invalidated']];
            $this->logger->debug("Found dangling invalidation, inserting {invalidationToInsert}", ['invalidationToInsert' => json_encode($invalidationToInsert)]);
        ...
        \Piwik\Db\BatchInsert::tableInsertBatch(\Piwik\Common::prefixTable('archive_invalidations'), $fields, $invalidationsToInsert);
    }
    private function setInvalidationTime()
    {
        $cache = \Piwik\Cache::getTransientCache();
        \Piwik\Option::set(self::CRON_INVALIDATION_TIME_OPTION_NAME, time());
    ...
    public static function getLastInvalidationTime()
    {
        ...
        \Piwik\Option::clearCachedOption(self::CRON_INVALIDATION_TIME_OPTION_NAME);
        $result = \Piwik\Option::get(self::CRON_INVALIDATION_TIME_OPTION_NAME);
        if (empty($result)) {
            \Piwik\Option::clearCachedOption(self::OPTION_ARCHIVING_FINISHED_TS);
            $result = \Piwik\Option::get(self::OPTION_ARCHIVING_FINISHED_TS);
        }
    ...
    private function logInitInfo()
    {
        ...
        $this->logger->info("Running Matomo " . \Piwik\Version::VERSION . " as Super User");
    }
    private function logArchiveTimeoutInfo()
    {
        ...
        $cliMulti = new \Piwik\CliMulti($this->logger);
        ...
        $this->logger->info("- Reports for today will be processed at most every " . $this->todayArchiveTimeToLive . " seconds. You can change this value in Matomo UI > Settings > General Settings.");
        foreach (['week', 'month', 'year', 'range'] as $period) {
            ...
            if (!empty($ttl) && $ttl !== $this->todayArchiveTimeToLive) {
                $this->logger->info("- Reports for the current {$period} will be processed at most every " . $ttl . " seconds. You can change this value in config/config.ini.php by editing 'time_before_" . $period . "_archive_considered_outdated' in the '[General]' section.");
            }
        }
        ...
        if ($this->lastSuccessRunTimestamp !== \false) {
            ...
            $this->logger->info("- Archiving was last executed without error " . $this->formatter->getPrettyTimeFromSeconds($dateLast, \true) . " ago.");
        }
    }
    ...
    {
        $timestamp = \Piwik\Option::get(self::OPTION_ARCHIVING_FINISHED_TS);
    ...
    private function getCustomDateRangeToPreProcess($idSite)
    {
        ...
        $customRanges = array_filter(\Piwik\Config::getInstance()->General['archiving_custom_ranges']);
    ...
    private function loadCustomDateRangeToPreProcess()
    {
        ...
        $allUsersPreferences = APIUsersManager::getInstance()->getAllUsersPreferences([APIUsersManager::PREFERENCE_DEFAULT_REPORT_DATE, APIUsersManager::PREFERENCE_DEFAULT_REPORT]);
    ...
    private function makeRequestUrl($url)
    {
         ...
         * @ignore
         */
        \Piwik\Piwik::postEvent('CronArchive.alterArchivingRequestUrl', [&$url]);
        return $url;
    }
    /**
     * @return CliMulti
    ...
    private function hasReachedMaxConcurrentArchivers()
    {
        ...
        if ($this->maxConcurrentArchivers && $cliMulti->supportsAsync()) {
            ...
            $instanceId = \Piwik\SettingsPiwik::getPiwikInstanceId();
            foreach ($processes as $process) {
                if (strpos($process, ' core:archive') !== \false && strpos($process, 'console ') !== \false && (!$instanceId || strpos($process, '--matomo-domain=' . $instanceId) !== \false || strpos($process, '--matomo-domain="' . $instanceId . '"') !== \false || strpos($process, '--matomo-domain=\'' . $instanceId . "'") !== \false || strpos($process, '--piwik-domain=' . $instanceId) !== \false || strpos($process, '--piwik-domain="' . $instanceId . '"') !== \false || strpos($process, '--piwik-domain=\'' . $instanceId . "'") !== \false)) {
                    $numRunning++;
                }
            }
    ...
    protected function wasSegmentChangedRecently($definition, $allSegments)
    {
        foreach ($allSegments as $segment) {
            if ($segment['definition'] === $definition) {
                $twentyFourHoursAgo = \Piwik\Date::now()->subHour(24);
                $segmentDate = $segment['ts_created'];
                if (!empty($segment['ts_last_edit'])) {
                    $segmentDate = $segment['ts_last_edit'];
                }
                return \Piwik\Date::factory($segmentDate)->isLater($twentyFourHoursAgo);
            }
        }
        return \false;
    }
