...
class CacheId
{
    ...
    {
        if (empty($_GET[$queryParamName]) && empty($_POST[$queryParamName])) {
            return [];
        }
        $idSiteGetParam = [];
    ...
    private static function idSiteListCacheKey($idSites)
    {
        ...
        if (count($idSites) <= 5) {
            return '-' . implode('_', $idSites);
            // we keep the cache key readable when possible
        } else {
            return '-' . md5(implode('_', $idSites));
