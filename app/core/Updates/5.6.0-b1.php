...
class Updates_5_6_0_b1 extends PiwikUpdates
{
    ...
    public function getMigrations(Updater $updater)
    {
        ...
        $invalidationStartDate = Db::fetchOne('SELECT DATE(created_time) FROM ' . Common::prefixTable('changes') . ' WHERE plugin_name = ? AND version = ? LIMIT 1', ['Referrers', '5.5.0-b1']);
        $migrations = [];
        if ($invalidationStartDate) {
            $commandToExecute = sprintf('./console core:invalidate-report-data --dates=%s,today --plugin=Referrers.' . Archiver::AI_ASSISTANTS_ENTRY_URL_RECORD_NAME, $invalidationStartDate);
