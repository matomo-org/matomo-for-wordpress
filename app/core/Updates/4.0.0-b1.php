...
class Updates_4_0_0_b1 extends PiwikUpdates
{
    ...
    public function getMigrations(Updater $updater)
    {
        ...
        foreach ($columnsToAdd as $table => $columns) {
            ...
            foreach ($columns as $columnName => $columnType) {
                ...
                if ($table == 'log_visit' && isset($columnsToAdd['log_conversion'][$columnName])) {
                    $optionValue .= '1';
                    // column is in log_conversion too
                }
                $migrations[] = $this->migration->db->sql("INSERT IGNORE INTO `" . Common::prefixTable('option') . "` (option_name, option_value) VALUES ('{$optionKey}', '{$optionValue}')");
            }
        }
        ...
        if ($hasDaysColumnInVisit) {
            $migrations[] = $this->migration->db->sql("UPDATE " . Common::prefixTable('log_visit') . " SET visitor_seconds_since_first = visitor_days_since_first * 86400, \n                    visitor_seconds_since_order = visitor_days_since_order * 86400,\n                    visitor_seconds_since_last = visitor_days_since_last * 86400");
        }
        ...
        if ($hasDaysColumnInConv) {
            $migrations[] = $this->migration->db->sql("UPDATE " . Common::prefixTable('log_conversion') . " SET visitor_seconds_since_first = visitor_days_since_first * 86400, \n                    visitor_seconds_since_order = visitor_days_since_order * 86400");
        }
        // remove old days_to_... columns
        $migrations[] = $this->migration->db->dropColumns('log_visit', ['config_gears', 'config_director', 'visitor_days_since_first', 'visitor_days_since_order', 'visitor_days_since_last']);
        $migrations[] = $this->migration->db->dropColumns('log_conversion', ['visitor_days_since_first', 'visitor_days_since_order']);
    ...
    public static function ensureCorePluginsThatWereMovedToMarketplaceCanBeUpdated()
    {
        ...
        foreach ($plugins as $plugin) {
            if ($pluginManager->isPluginThirdPartyAndBogus($plugin)) {
                ...
                if (is_dir($pluginDir) && file_exists($pluginDir . '/' . $plugin . '.php') && !file_exists($pluginDir . '/plugin.json') && is_writable($pluginDir)) {
  ...
  "keywords": ["' . $plugin . '"]
                    ...
                    $pluginManager->unloadPlugin($pluginObj);
                    // prevent any events being posted to it somehow
                }
            }
        }
    }
