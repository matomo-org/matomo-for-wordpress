...
class VisitorRecognizer
{
    ...
    public function findKnownVisitor($configId, VisitProperties $visitProperties, \Piwik\Tracker\Request $request)
    {
    ...
    public function removeUnchangedValues($visit, ?VisitProperties $originalVisit = null)
    {
        ...
        if (!empty($originalRow['idvisitor']) && !empty($visit['idvisitor']) && bin2hex($originalRow['idvisitor']) === bin2hex($visit['idvisitor'])) {
        ...
        foreach ($fieldsToCompareValue as $field) {
            if (!empty($originalRow[$field]) && !empty($visit[$field]) && $visit[$field] == $originalRow[$field]) {
