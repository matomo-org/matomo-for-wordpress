 ...
 *
 */
class ActionPageview extends \Piwik\Tracker\Action
{
    public function __construct(\Piwik\Tracker\Request $request)
    {
        parent::__construct(\Piwik\Tracker\Action::TYPE_PAGE_URL, $request);
    ...
    {
        return array('idaction_name' => array($this->getActionName(), \Piwik\Tracker\Action::TYPE_PAGE_TITLE), 'idaction_url' => $this->getUrlAndType());
    }
    public static function shouldHandle(\Piwik\Tracker\Request $request)
    {
        return !\Piwik\Tracker\Action::isCustomActionRequest($request);
    }
