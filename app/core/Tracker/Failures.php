...
class Failures
{
    ...
    public function logFailure($idFailure, \Piwik\Tracker\Request $request)
    {
        $isVisitExcluded = $request->getMetadata('CoreHome', 'isVisitExcluded');
        if ($isVisitExcluded === null) {
            try {
                $visitExcluded = new \Piwik\Tracker\VisitExcluded($request);
    ...
    private function getParamsWithTokenAnonymized(\Piwik\Tracker\Request $request)
    {
        ...
        foreach ($params as $key => $value) {
            if (!empty($token) && $value === $token) {
                $params[$key] = '__TOKEN_AUTH__';
                // user accidentally posted the token in a wrong field
            } elseif (!empty($value) && is_string($value) && mb_strlen($value) >= 29 && mb_strlen($value) <= 36 && ctype_xdigit($value)) {
                $params[$key] = '__TOKEN_AUTH__';
                // user maybe posted a token in a different field... it looks like it might be a token
            }
        }
        return $params;
    }
    ...
    private function enrichFailures($failures)
    {
        foreach ($failures as &$failure) {
            ...
            if (empty($params['url'])) {
                $params['url'] = ' ';
                // workaround it using the default provider in request constructor
            }
            $request = new \Piwik\Tracker\Request($params);
