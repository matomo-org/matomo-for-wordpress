...
class GoalManager
{
    ...
    protected function getRevenue($revenue)
    {
        ...
        if (abs((float) $revenue) > self::MAX_ALLOWED_REVENUE) {
            StaticContainer::get(LoggerInterface::class)->debug("Ecommerce value ({$revenue}) exceeds the allowed maximum of " . self::MAX_ALLOWED_REVENUE . " and was rejected (treated as no revenue).");
            return 0;
        }
