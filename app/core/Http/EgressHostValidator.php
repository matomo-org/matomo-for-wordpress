...
class EgressHostValidator
{
    ...
    private const EXTRA_BLOCKED_RANGES = ['100.64.0.0/10', '192.0.0.0/24', '192.0.2.0/24', '192.88.99.0/24', '198.18.0.0/15', '198.51.100.0/24', '203.0.113.0/24', '224.0.0.0/4', '::/96', '2002::/16', '2001::/32', '2001:db8::/32', '64:ff9b::/96', '64:ff9b:1::/48', '100::/64', 'fe00::/8', 'ff00::/8'];
    ...
    public function __construct(?callable $resolver = null, ?array $allowedPrivateRanges = null)
    {
        ...
        $this->allowedPrivateRanges = $allowedPrivateRanges ?? GeneralConfig::getArrayConfigValue('allowed_private_egress_ranges', []);
    }
    /**
    ...
    public function resolveTarget(string $host) : array
    {
        $host = trim($host, '[]');
        if ($host === '') {
            throw new \Piwik\Http\EgressBlockedException('Refusing to fetch: empty host.');
        }
        // Fold IDN to the ASCII form the transport parses, so validation and pin match it.
        if (preg_match('/[^\\x20-\\x7e]/', $host)) {
            if (!function_exists('idn_to_ascii')) {
                throw new \Piwik\Http\EgressBlockedException('Refusing to fetch: cannot normalise internationalised host without the intl extension.');
            }
            $ascii = idn_to_ascii($host, \IDNA_DEFAULT, \INTL_IDNA_VARIANT_UTS46);
            if ($ascii === \false || $ascii === '') {
                throw new \Piwik\Http\EgressBlockedException('Refusing to fetch: host cannot be converted to ASCII.');
            }
            $host = $ascii;
        }
        ...
        if (filter_var($host, \FILTER_VALIDATE_IP)) {
            if (!$this->isAllowedIp($host)) {
                throw new \Piwik\Http\EgressBlockedException('Refusing to fetch: host resolves to a private or reserved address.');
            }
            return [$host, $host];
        }
        ...
        if (preg_match('/^0x[0-9a-f]+$/i', $host) || preg_match('/^[0-9.]+$/', $host)) {
            throw new \Piwik\Http\EgressBlockedException('Refusing to fetch: numeric or encoded host is not allowed.');
        }
        if (!preg_match('/^(?=.{1,253}$)([a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?)(\\.[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?)*$/i', $host)) {
            throw new \Piwik\Http\EgressBlockedException('Refusing to fetch: host is not a valid IP or DNS name.');
        }
        $ips = ($this->resolver)($host);
        if (empty($ips)) {
            throw new \Piwik\Http\EgressBlockedException('Refusing to fetch: host could not be resolved.');
        }
        foreach ($ips as $ip) {
    ...
    private function isAllowedIp(string $ip) : bool
    {
        ...
        return !empty($this->allowedPrivateRanges) && IP::fromStringIP($ip)->isInRanges($this->allowedPrivateRanges);
    }
