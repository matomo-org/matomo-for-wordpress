...
class Segment
{
    ...
    protected function getCleanedExpression(array $expression) : array
    {
        ...
        if ($segmentObject && $segmentObject->dimension && $segmentObject->dimension->getDbColumnJoin()) {
            ...
            $sourceTable = strpos((string) $sqlName, '.') !== \false ? strstr($sqlName, '.', \true) : null;
            if ($sourceTable !== null) {
                foreach ($join->getAdditionalKeyColumns() as $keyColumn) {
                    $joinConditions[] = $sourceTable . '.' . $keyColumn . ' = ' . $tableAlias . '.' . $keyColumn;
                }
            }
            $joinTable = ['table' => $join->getTable(), 'tableAlias' => $tableAlias, 'field' => $tableAlias . '.' . $join->getTargetColumn(), 'joinOn' => implode(' AND ', $joinConditions)];
            if ($dbDiscriminator) {
