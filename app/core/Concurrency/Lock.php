...
class Lock
{
    ...
    public function __construct(\Piwik\Concurrency\LockBackend $backend, $lockKeyStart, $defaultTtl = null)
    {
    ...
    public function reexpireLock()
    {
        ...
        if (!empty($this->lastExpireTime) && $now <= $this->lastExpireTime + $timeBetweenReexpires) {
            return false;
        }
    ...
    public function execute($id, $callback)
    {
        ...
        while (!$this->acquireLock($id)) {
            ...
            // 100ms
            if ($i > 50) {
                // give up after 5seconds (50 * 100ms)
