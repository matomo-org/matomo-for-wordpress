...
class ArchiveTableCreator
{
    ...
    private static function triggerLegacyDefaultDeprecation(string $type) : void
    {
        ...
        trigger_error(sprintf("Omitting \$createIfMissing in ArchiveTableCreator::%s() is deprecated. Pass true to create missing archive tables or false to only return existing ones. In Matomo 6 the default behavior will change to lookup-only.", $method), \E_USER_DEPRECATED);
    }
