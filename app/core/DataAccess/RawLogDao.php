...
class RawLogDao
{
    ...
    {
        $sql = "UPDATE " . Common::prefixTable('log_visit') . " SET " . $this->getColumnSetExpressions(array_keys($values)) . " WHERE idvisit = ?";
    ...
    {
        $sql = "UPDATE " . Common::prefixTable('log_conversion') . " SET " . $this->getColumnSetExpressions(array_keys($values)) . " WHERE idvisit = ?";
        $this->update($sql, $values, $idVisit);
    }
    /**
     * @param string $from
    ...
    {
        $columnsToSet = array_map(function ($column) {
            return $column . ' = ?';
        }, $columnsToSet);
    ...
    private function lockLogTables()
    {
        ...
        Db::lockTables($readLocks, $writeLocks = Common::prefixTables('log_action'));
    }
