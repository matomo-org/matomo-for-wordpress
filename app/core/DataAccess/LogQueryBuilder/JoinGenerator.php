...
class JoinGenerator
{
    ...
    public function __construct(\Piwik\DataAccess\LogQueryBuilder\JoinTables $tables)
    {
        $this->tables = $tables;
    ...
    public function generate()
    {
        ...
        foreach ($this->tables as $i => $table) {
            ...
            if (is_array($table)) {
                ...
                $this->joinString .= ' ' . Common::prefixTable($table['table']) . " AS " . $alias . " ON " . $table['joinOn'];
                continue;
            }
            ...
            if ($i == 0) {
                ...
                $joinName = 'LEFT JOIN';
                if ($i > 0 && $this->tables[$i - 1] && is_string($this->tables[$i - 1]) && strpos($this->tables[$i - 1], LogAggregator::LOG_TABLE_SEGMENT_TEMPORARY_PREFIX) === 0) {
                    $joinName = 'INNER JOIN';
    ...
    public function findJoinCriteriasForTables(LogTable $logTable, $availableLogTables)
    {
        ...
        if ($this->tables->hasJoinedTableManually($table, $join) || $this->tables->hasJoinedTableManually($table, $alternativeJoin)) {
            // already joined, no need to join it again
            return null;
        }
        if ($table == 'log_conversion_item') {
            // by default we don't want to consider deleted columns
