...
class Model
{
        ...
        foreach ($rows as $row) {
            ...
            while (!empty($duplicateArchives)) {
                ...
                if ($this->isCutOffGroupConcatResult($pair)) {
                    // can occur if the GROUP_CONCAT value is cut off
                    break;
                }
                [$idarchive, $value] = explode('.', $pair);
                array_shift($duplicateArchives);
                if ($value != \Piwik\DataAccess\ArchiveWriter::DONE_PARTIAL) {
                    break;
                }
            }
            ...
            if (!empty($duplicateArchives)) {
                foreach ($duplicateArchives as $pair) {
                    ...
                    // does not matter what the value is, the latest is usable so older archives can be purged
                }
            }
        }
        return $archiveIds;
    ...
    public function updateArchiveAsInvalidated($archiveTable, $idSites, $allPeriodsToInvalidate, ?Segment $segment = null, bool $forceInvalidateNonexistentRanges = \false, ?string $name = null, bool $doNotCreateInvalidations = \false)
    {
        ...
        if (!empty($allPeriodsToInvalidate)) {
            ...
            foreach ($allPeriodsToInvalidate as $period) {
                ...
                if ($period->getLabel() == 'range') {
                    // for ranges, we delete all ranges that contain the given date(s)
                    $periodCondition .= "(period = " . (int) $period->getId() . " AND date2 >= '" . $period->getDateStart()->getDatetime() . "' AND date1 <= '" . $period->getDateEnd()->getDatetime() . "')";
                } else {
                    $periodCondition .= "(period = " . (int) $period->getId() . " AND date1 = '" . $period->getDateStart()->getDatetime() . "'" . " AND date2 = '" . $period->getDateEnd()->getDatetime() . "')";
                }
            }
            $periodCondition .= ")";
        }
        ...
        if (empty($plugin)) {
            if (null === $segment) {
                ...
                // invalidate specific segment only
            }
        } else {
            if (null === $segment) {
                $nameCondition = "name LIKE '{$doneFlag}%.{$plugin}'";
                // invalidate all segments for specific plugin
            } else {
        ...
        if (empty($name)) {
            ...
            if (!empty($idArchives)) {
                ...
                $sql = "UPDATE `{$archiveTable}` SET `value` = " . \Piwik\DataAccess\ArchiveWriter::DONE_INVALIDATED . " WHERE idarchive IN (" . implode(',', $idArchives) . ") AND value NOT IN (" . \Piwik\DataAccess\ArchiveWriter::DONE_ERROR . ", " . \Piwik\DataAccess\ArchiveWriter::DONE_ERROR_INVALIDATED . ") AND {$nameCondition}";
                Db::query($sql);
        ...
        foreach ($idSites as $idSite) {
            ...
            foreach ($allPeriodsToInvalidate as $period) {
                if ($period->getLabel() == 'range' && !$forceInvalidateNonexistentRanges) {
                    continue;
                    // range
                }
                if ($period->getDateEnd()->isEarlier($siteCreationTime)) {
                    continue;
                    // don't add entries if it is before the time the site was created
                }
                ...
                foreach ($doneFlagsToCheck as $doneFlagToCheck) {
                    ...
                    $dummyArchives[] = ['idarchive' => $idArchive, 'name' => $doneFlagToCheck, 'report' => $name, 'idsite' => $idSite, 'date1' => $period->getDateStart()->getDatetime(), 'date2' => $period->getDateEnd()->getDatetime(), 'period' => $period->getId(), 'ts_invalidated' => $now];
                }
            }
        }
    ...
    public function getArchiveIdAndVisits($numericTable, $idSite, $period, $dateStartIso, $dateEndIso, $minDatetimeIsoArchiveProcessedUTC, $doneFlags, $doneFlagValues = null)
    {
    ...
    public function createArchiveTable($tableName, $tableNamePrefix)
    {
        ...
        try {
            if (\Piwik\DataAccess\ArchiveTableCreator::NUMERIC_TABLE === \Piwik\DataAccess\ArchiveTableCreator::getTypeFromTableName($tableName)) {
    ...
    {
        $rows = Db::fetchAll("SELECT DISTINCT idsite FROM `{$numericTable}` WHERE `name` LIKE 'done%' AND `value` IN (" . \Piwik\DataAccess\ArchiveWriter::DONE_INVALIDATED . ")");
        $result = array();
        foreach ($rows as $row) {
            $result[] = $row['idsite'];
        }
    ...
    public function startArchive($invalidation)
    {
        ...
        $statement = Db::query("UPDATE `{$table}` SET `status` = ?, `processing_host` = ?, `process_id` = ?, `ts_started` = NOW() WHERE `idinvalidation` = ? AND `status` = ?", [ArchiveInvalidator::INVALIDATION_STATUS_IN_PROGRESS, gethostname() ?: null, Common::getProcessId(), $invalidation['idinvalidation'], ArchiveInvalidator::INVALIDATION_STATUS_QUEUED]);
    ...
    {
        $table = Common::prefixTable('archive_invalidations');
        $bind = [$invalidation['idsite'], $invalidation['period'], $invalidation['date1'], $invalidation['date2'], $invalidation['name'], ArchiveInvalidator::INVALIDATION_STATUS_IN_PROGRESS];
    ...
    public function getInvalidationsInProgress(array $idSites = [], array $processingHosts = [], ?Date $startTime = null, ?Date $endTime = null) : array
    {
        $table = Common::prefixTable('archive_invalidations');
    ...
    public function getNextInvalidatedArchive($idSite, $archivingStartTime, $idInvalidationsToExclude = null, $useLimit = \true)
    {
        ...
        $bind = [$idSite, ArchiveInvalidator::INVALIDATION_STATUS_IN_PROGRESS, $archivingStartTime];
    ...
    {
        $table = Common::prefixTable('archive_invalidations');
        $bind = [$invalidatedArchive['idsite'], $invalidatedArchive['date1'], $invalidatedArchive['date2'], $invalidatedArchive['period'], $invalidatedArchive['name']];
    ...
    public function hasChildArchivesInPeriod($idSite, Period $period)
    {
        ...
        while ($date->isEarlier($period->getDateEnd()->addPeriod(1, 'month'))) {
            ...
            $usableDoneFlags = [\Piwik\DataAccess\ArchiveWriter::DONE_OK, \Piwik\DataAccess\ArchiveWriter::DONE_INVALIDATED, \Piwik\DataAccess\ArchiveWriter::DONE_PARTIAL, \Piwik\DataAccess\ArchiveWriter::DONE_OK_TEMPORARY];
    ...
    public function hasInvalidationForPeriodAndName($idSite, Period $period, $doneFlag, $report = null)
    {
        ...
        $bind = [$idSite, $period->getDateStart()->toString(), $period->getDateEnd()->toString(), $period->getId(), $doneFlag];
        if (!empty($report)) {
