...
class Translator
{
    ...
    public function getJavascriptTranslations()
    {
        ...
        foreach ($this->getClientSideTranslationKeys() as $id) {
            if (strpos($id, '_') === \false) {
                StaticContainer::get(LoggerInterface::class)->warning('Unexpected translation key found in client side translations: {translation_key}', ['translation_key' => $id]);
                continue;
            }
    ...
    private function getTranslation($id, $lang, $plugin, $key)
    {
        ...
        if (isset($this->translations[$lang][$plugin]) && isset($this->translations[$lang][$plugin][$key])) {
         ...
         */
        if ($plugin != 'Intl') {
            if (isset($this->translations[$lang]['Intl']) && isset($this->translations[$lang]['Intl'][$key])) {
