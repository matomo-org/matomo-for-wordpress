...
class Pdf extends ReportRenderer
{
        ...
        foreach ($this->report->getRows() as $rowId => $row) {
            ...
            foreach ($this->reportColumns as $columnId => $columnName) {
                ...
                if ($columnId == 'label') {
                    ...
                    $this->TCPDF->MultiCell($this->labelCellWidth, $rowHeight, $text, 'LR', 'L', $fill, 0, '', '', \true, 0, \false, \true, $maxHeight, $verticalAlign);
                    ...
                    if (!$metricsPaddingApplied) {
                        ...
                        $metricsPaddingApplied = \true;
                    }
                    $this->TCPDF->Cell($columnWidth, $rowHeight, NumberFormatter::getInstance()->format($rowMetrics[$columnId]), 'LR', 0, 'L', $fill, '', 0, \false, 'T', 'T');
                }
            }
            if ($metricsPaddingApplied) {
    ...
    private function computeLabelRenderState(array $rowMetrics, array $rowMetadata, string $leftSpacesBeforeLogo, &$url) : array
    {
        ...
        $maxHeight = $shouldIncreaseLineHeight ? $rowHeight : $this->getLabelRowMaxHeight($rowHeight);
        $verticalAlign = $shouldIncreaseLineHeight ? 'T' : 'M';
        return array('text' => $labelText, 'rowHeight' => $rowHeight, 'maxHeight' => $maxHeight, 'verticalAlign' => $verticalAlign, 'shouldIncreaseLineHeight' => $shouldIncreaseLineHeight, 'isLogoDisplayable' => $isLogoDisplayable);
    }
    ...
    private function restoreLabelCellStyle(bool $shouldIncreaseLineHeight, ?float $previousCellHeightRatio, ?array $previousCellPaddingForLabel) : void
    {
    ...
    private function applyCellPaddingOffset(float $left = 0, float $top = 0.8) : array
    {
        ...
        $this->TCPDF->setCellPaddings($previousCellPadding['L'] + $left, $previousCellPadding['T'] + $top, $previousCellPadding['R'], $previousCellPadding['B']);
        return $previousCellPadding;
    }
    private function restoreCellPaddingOffset(array $previousCellPaddings) : void
    {
        $this->TCPDF->setCellPaddings($previousCellPaddings['L'], $previousCellPaddings['T'], $previousCellPaddings['R'], $previousCellPaddings['B']);
    }
    /**
    ...
    private function getLabelRowHeight(string $text) : float
    {
        ...
        $labelHeight = ceil($labelHeight * 2) / 2;
        // round up to nearest 0.5 for stable row heights
     ...
     */
    private function renderLabelLinkAndLogo($url, float $posX, float $posY, float $rowHeight, array $rowMetadata, bool $isLogoDisplayable, float &$logoWidth, float &$logoHeight) : void
    {
        if ($url) {
    ...
    {
        if (!empty($this->tableWidthCache['maxSingleLineLabelWidthFor']) && (float) $this->tableWidthCache['maxSingleLineLabelWidthFor'] === (float) $this->labelCellWidth) {
        ...
        foreach ($this->report->getRows() as $rowId => $row) {
            ...
            $labelText = $this->buildFormattedLabelTextForRow($rowId, (string) $label, $rowsMetadata, $this->maxLabelCharacter);
    ...
    {
        if (!empty($this->tableWidthCache) && isset($this->tableWidthCache['metricMaxWidths'][$columnId])) {
    ...
    {
        $this->tableWidthCache = array('ready' => \true, 'labelTooLong' => \false, 'labelFitsShortWidth' => \true, 'maxLabelLength' => 0, 'metricMaxWidths' => array(), 'maxSingleLineLabelWidth' => null, 'maxSingleLineLabelWidthFor' => null);
        if (!$this->reportHasData()) {
            return;
        }
        $this->TCPDF->SetFont($this->reportFont, $this->reportFontStyle, $this->reportSimpleFontSize);
        ...
        foreach ($this->report->getRows() as $rowId => $row) {
            ...
            if ($label !== \false && $label !== null) {
                ...
                if ($length > $maxCharacters) {
                    $this->tableWidthCache['labelTooLong'] = \true;
                }
                if ($this->tableWidthCache['labelFitsShortWidth']) {
                    $formattedLabel = $this->buildFormattedLabelTextForRow($rowId, $rawLabel, $rowsMetadata, $maxCharacters);
    ...
    private function buildFormattedLabelTextForRow(int $rowId, string $label, array $rowsMetadata, int $maxCharacters) : string
    {
    ...
    {
        $imageGraph = parent::getStaticGraph($this->reportMetadata, self::IMAGE_GRAPH_WIDTH_PORTRAIT, self::IMAGE_GRAPH_HEIGHT, $this->evolutionGraph, $this->segment);
    ...
    private function buildHeaderColumnData() : array
    {
        ...
        foreach ($this->reportColumns as $columnId => $columnName) {
            ...
            $columnData[] = array('text' => $columnName, 'width' => $columnWidth, 'height' => $cellHeight, 'textHeight' => $textHeight);
    ...
    private function renderHeaderColumns(array $columnData, float $initPosX, float $posY, float $maxCellHeight) : void
    {
        ...
        foreach ($columnData as $columnInfo) {
            ...
            $this->TCPDF->MultiCell($columnWidth, $this->cellHeight, $columnInfo['text'], 0, 'L', \false, 0, '', '', \true, 0, \false, \true, 0, 'T');
