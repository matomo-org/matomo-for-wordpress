...
abstract class RecordBuilder
{
    ...
    public function buildForNonDayPeriod(ArchiveProcessor $archiveProcessor) : void
    {
        ...
        $recordsBuilt = $this->getRecordMetadata($archiveProcessor);
        $numericRecords = array_filter($recordsBuilt, function (\Piwik\ArchiveProcessor\Record $r) {
            return $r->getType() == \Piwik\ArchiveProcessor\Record::TYPE_NUMERIC;
        });
        $blobRecords = array_filter($recordsBuilt, function (\Piwik\ArchiveProcessor\Record $r) {
            return $r->getType() == \Piwik\ArchiveProcessor\Record::TYPE_BLOB;
        });
        $blobRecordsByName = [];
        ...
        foreach ($blobRecords as $record) {
            ...
            if (empty($flatRecordName) || !in_array($flatRecordName, $requestedReports)) {
                continue;
            }
        ...
        foreach ($numericRecords as $record) {
            if (empty($record->getCountOfRecordName()) || !in_array($record->getName(), $requestedReports)) {
                continue;
            }
        ...
        foreach ($blobRecords as $record) {
            if (!empty($requestedReports) && (!in_array($record->getName(), $requestedReports) || in_array($record->getName(), $foundRequestedReports))) {
                continue;
            }
            ...
            if ($this->aggregateBuiltFromFlatRecordForNonDay($archiveProcessor, $record, $blobRecordsByName, $columnAggregationOps, $columnToRenameAfterAggregation, $columnToSortByBeforeTruncation, $processedFlatRecords)) {
                continue;
            }
            ...
            $postAggregationTransform = $recordTransform === null ? null : function (DataTable $table) use($recordTransform, $archiveProcessor, $record) : void {
                $recordTransform($table, $archiveProcessor, $record);
            };
            $counts = $archiveProcessor->aggregateDataTableRecords($record->getName(), $maxRowsInTable, $maxRowsInSubtable, $columnToSortByBeforeTruncation, $columnAggregationOps, $columnToRenameAfterAggregation, $countRecursiveRows, $countLeafRows, $postAggregationTransform);
        ...
        if (!empty($numericRecords)) {
            ...
            $autoAggregateMetrics = array_filter($numericRecords, function (\Piwik\ArchiveProcessor\Record $r) {
                return empty($r->getCountOfRecordName());
            });
            $autoAggregateMetrics = array_map(function (\Piwik\ArchiveProcessor\Record $r) {
                return $r->getName();
            ...
            $recordCountMetrics = array_filter($numericRecords, function (\Piwik\ArchiveProcessor\Record $r) {
                return !empty($r->getCountOfRecordName());
            });
    ...
    protected function aggregateBuiltFromFlatRecordForNonDay(ArchiveProcessor $archiveProcessor, \Piwik\ArchiveProcessor\Record $hierarchicalRecord, array $blobRecordsByName, ?array $columnAggregationOps, ?array $columnToRenameAfterAggregation, ?string $columnToSortByBeforeTruncation, array &$processedFlatRecords) : bool
    {
        ...
        $flatSerialized = $flatTable->getSerialized($flatMaxRowsInTable, null, $flatColumnToSortByBeforeTruncation);
        $archiveProcessor->insertBlobRecord($flatRecordName, $flatSerialized);
        unset($flatSerialized);
        $processedFlatRecords[$flatRecordName] = \true;
        $hierarchicalTable = $this->buildHierarchicalTableFromFlatTableAndConsumeRows($flatTable, $columnAggregationOps, function (Row $flatRow) use($flatToHierarchyPathCallback, $archiveProcessor, $hierarchicalRecord) {
        ...
        $hierarchicalSerialized = $hierarchicalTable->getSerialized(null, null, $columnToSortByBeforeTruncation);
     ...
     */
    protected function beforeInsertBuiltFromFlatHierarchyRecord(ArchiveProcessor $archiveProcessor, \Piwik\ArchiveProcessor\Record $hierarchicalRecord, DataTable $hierarchicalTable, DataTable $flatTable) : void
    {
    }
    protected function buildHierarchicalTableFromFlatTable(DataTable $flatTable, ?array $columnAggregationOps, callable $flatToHierarchyPathCallback, array $defaultHierarchyRowColumns = []) : DataTable
    {
    ...
    protected function buildHierarchicalTableFromFlatTableAndConsumeRows(DataTable $flatTable, ?array $columnAggregationOps, callable $flatToHierarchyPathCallback, array $defaultHierarchyRowColumns = []) : DataTable
    {
        $hierarchicalTable = new DataTable();
        if (!empty($columnAggregationOps)) {
            $hierarchicalTable->setMetadata(DataTable::COLUMN_AGGREGATION_OPS_METADATA_NAME, $columnAggregationOps);
    ...
    protected function sumRowIntoDestination(Row $source, Row $destination, ?array $columnAggregationOps) : void
    {
        ...
        $destination->sumRow($sourceCopy, \true, $columnAggregationOps ?? []);
    }
