...
class Loader
{
    ...
    public function __construct(\Piwik\ArchiveProcessor\Parameters $params, $invalidateBeforeArchiving = \false)
    {
        $this->params = $params;
    ...
    private function prepareArchiveImpl($pluginName)
    {
        ...
        if ($this->invalidateBeforeArchiving && \Piwik\ArchiveProcessor\Rules::isBrowserTriggerEnabled()) {
        ...
        if (ArchiveProcessor::$isRootArchivingRequest && !SettingsServer::isArchivePhpTriggered()) {
            ...
            //ini lock
            $lock = new \Piwik\ArchiveProcessor\LoaderLock($lockId);
    ...
    {
        $doneFlag = \Piwik\ArchiveProcessor\Rules::getDoneStringFlagFor([$this->params->getSite()->getId()], $this->params->getSegment(), $this->params->getPeriod()->getLabel(), $this->params->getRequestedPlugin());
    ...
    protected function loadArchiveData()
    {
        ...
        if (!empty($idArchives) && !\Piwik\ArchiveProcessor\Rules::isActuallyForceArchivingSinglePlugin() && !$this->shouldForceInvalidatedArchive($doneFlagValue, $tsArchived) && !$isMissingRequestedRecords) {
    ...
    {
        $processAllReportsIncludingVisitsSummary = \Piwik\ArchiveProcessor\Rules::shouldProcessReportsAllPlugins(array($this->params->getSite()->getId()), $this->params->getSegment(), $this->params->getPeriod()->getLabel());
    ...
    public function getReportsToInvalidate()
    {
        ...
        foreach ($sitesPerDays as $dateStr => $siteIds) {
            if (empty($siteIds) || !in_array($this->params->getSite()->getId(), $siteIds)) {
                unset($sitesPerDays[$dateStr]);
            }
            $date = Date::factory($dateStr);
            if ($date->isEarlier($this->params->getPeriod()->getDateStart()) || $date->isLater($this->params->getPeriod()->getDateEnd())) {
