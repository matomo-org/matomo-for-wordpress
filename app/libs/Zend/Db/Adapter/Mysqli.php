...
namespace {
    ...
    class Zend_Db_Adapter_Mysqli extends \Zend_Db_Adapter_Abstract
    {
        ...
        public function describeTable($tableName, $schemaName = null)
        {
             ...
             */
            if ($schemaName) {
                $sql = 'DESCRIBE ' . $this->quoteIdentifier("{$schemaName}.{$tableName}", \true);
            } else {
                $sql = 'DESCRIBE ' . $this->quoteIdentifier($tableName, \true);
            }
            /**
        ...
        protected function _connect()
        {
            ...
            $enable_ssl = \false;
            $ssl_options = array('ssl_ca' => null, 'ssl_ca_path' => null, 'ssl_cert' => null, 'ssl_cipher' => null, 'ssl_key' => null);
            if (!empty($this->_config['driver_options'])) {
                foreach ($this->_config['driver_options'] as $option => $value) {
                    if (\array_key_exists($option, $ssl_options)) {
                        ...
                        $enable_ssl = \true;
                        continue;
                        // these options are set below in mysqli_ssl_set
            ...
            if ($enable_ssl) {
                \mysqli_ssl_set($this->_connection, $ssl_options['ssl_key'], $ssl_options['ssl_cert'], $ssl_options['ssl_ca'], $ssl_options['ssl_ca_path'], $ssl_options['ssl_cipher']);
            }
            $flags = null;
            if ($enable_ssl) {
                $flags = \MYSQLI_CLIENT_SSL;
                if (!empty($this->_config['driver_options']['ssl_no_verify']) && \defined('MYSQLI_CLIENT_SSL_DONT_VERIFY_SERVER_CERT')) {
            ...
            $_isConnected = @\mysqli_real_connect($this->_connection, $this->_config['host'], $this->_config['username'], $this->_config['password'], $this->_config['dbname'], $port, $socket, $enable_ssl ? $flags : null);
        ...
        protected function _commit()
        {
            ...
            $this->_connection->autocommit(\true);
        }
        /**
