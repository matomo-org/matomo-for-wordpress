...
class System
{
    ...
    protected static function _dirToStruct($sPath, $maxinst, $aktinst = 0, $silent = \false)
    {
        ...
        if (($dir = @\opendir($sPath)) === \false) {
            if (!$silent) {
                \Matomo\Dependencies\System::raiseError("Could not open dir {$sPath}");
            }
            return $struct;
        ...
        if ($aktinst < $maxinst || $maxinst == 0) {
            foreach ($list as $val) {
                ...
                if (\is_dir($path) && !\is_link($path)) {
                    $tmp = \Matomo\Dependencies\System::_dirToStruct($path, $maxinst, $aktinst + 1, $silent);
    ...
    protected static function _multipleToStruct($files)
    {
        ...
        foreach ($files as $file) {
            if (\is_dir($file) && !\is_link($file)) {
                $tmp = \Matomo\Dependencies\System::_dirToStruct($file, 0);
    ...
    {
        $opts = \Matomo\Dependencies\System::_parseArgs($args, 'rf');
        // "f" does nothing but I like it :-)
        if (PEAR::isError($opts)) {
            return \Matomo\Dependencies\System::raiseError($opts);
        }
        ...
        if (isset($do_recursive)) {
            $struct = \Matomo\Dependencies\System::_multipleToStruct($opts[1]);
    ...
    {
        $opts = \Matomo\Dependencies\System::_parseArgs($args, 'pm:');
        if (PEAR::isError($opts)) {
            return \Matomo\Dependencies\System::raiseError($opts);
        }
        $mode = 0777;
    ...
    public static function &cat($args)
    {
        ...
        if (isset($mode)) {
            if (!($outputfd = \fopen($outputfile, $mode))) {
                $err = \Matomo\Dependencies\System::raiseError("Could not open {$outputfile}");
                return $err;
            }
            $ret = \true;
        }
        foreach ($files as $file) {
            if (!($fd = \fopen($file, 'r'))) {
                \Matomo\Dependencies\System::raiseError("Could not open {$file}");
                continue;
            }
    ...
    {
        static $first_time = \true;
        $opts = \Matomo\Dependencies\System::_parseArgs($args, 't:d');
        if (PEAR::isError($opts)) {
            return \Matomo\Dependencies\System::raiseError($opts);
        }
        foreach ($opts[0] as $opt) {
        ...
        if (!isset($tmpdir)) {
            $tmpdir = \Matomo\Dependencies\System::tmpdir();
        }
        if (!\Matomo\Dependencies\System::mkDir(array('-p', $tmpdir))) {
            return \false;
        }
        $tmp = \tempnam($tmpdir, $prefix);
        if (isset($tmp_is_dir)) {
            \unlink($tmp);
            // be careful possible race condition here
            if (!\mkdir($tmp, 0700)) {
                return \Matomo\Dependencies\System::raiseError("Unable to create temporary directory {$tmpdir}");
            }
        }
    ...
    public static function _removeTmpFiles()
    {
        if (\count($GLOBALS['_System_temp_files'])) {
            ...
            \Matomo\Dependencies\System::rm($delete);
            $GLOBALS['_System_temp_files'] = array();
        }
    }
    /**
    ...
    public static function find($args)
    {
        ...
        $path = \Matomo\Dependencies\System::_dirToStruct($dir, $depth, 0, \true);
        if ($do_files && $do_dirs) {
