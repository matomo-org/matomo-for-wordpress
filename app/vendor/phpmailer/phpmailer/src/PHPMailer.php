...
class PHPMailer
{
    ...
    protected function edebug($str)
    {
        ...
        if ($this->Debugoutput instanceof \Matomo\Dependencies\Psr\Log\LoggerInterface) {
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (false === $pos) {
            ...
            if ($this->exceptions) {
                throw new \PHPMailer\PHPMailer\Exception($error_message);
            }
            return false;
        }
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                throw new \PHPMailer\PHPMailer\Exception($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty && empty($this->Body)) {
                throw new \PHPMailer\PHPMailer\Exception($this->lang('empty_message'), self::STOP_CRITICAL);
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new \PHPMailer\PHPMailer\Exception($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new \PHPMailer\PHPMailer\Exception($this->lang('instantiate'), self::STOP_CRITICAL);
        }
        return true;
    }
    /**
    ...
    {
        if (!in_array($name, \PHPMailer\PHPMailer\SMTP::$xclient_allowed_attributes)) {
            return false;
        }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            throw new \PHPMailer\PHPMailer\Exception($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if (count($this->all_recipients) > count($bad_rcpt) && !$this->smtp->data($header . $body)) {
            throw new \PHPMailer\PHPMailer\Exception($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
        ...
        if (count($bad_rcpt) > 0) {
            ...
            throw new \PHPMailer\PHPMailer\Exception($this->lang('recipients_failed') . $errstr, self::STOP_CONTINUE);
        }
        return true;
    }
    /**
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            ...
                            throw new \PHPMailer\PHPMailer\Exception($message);
                        }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new \PHPMailer\PHPMailer\Exception($this->lang('extension_missing') . 'openssl');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new \PHPMailer\PHPMailer\Exception($this->lang('signing') . openssl_error_string());
                }
            } catch (\PHPMailer\PHPMailer\Exception $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = self::ENCODING_BASE64, $type = '', $disposition = 'attachment')
    {
        try {
            if (!static::fileIsAccessible($path)) {
                throw new \PHPMailer\PHPMailer\Exception($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
            ...
            if (!$this->validateEncoding($encoding)) {
                throw new \PHPMailer\PHPMailer\Exception($this->lang('encoding') . $encoding);
            }
            $this->attachment[] = [
                0 => $path,
                1 => $filename,
    ...
    protected function encodeFile($path, $encoding = self::ENCODING_BASE64)
    {
        try {
            if (!static::fileIsAccessible($path)) {
                throw new \PHPMailer\PHPMailer\Exception($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
        ...
        foreach ($this->CustomHeader as $k => $pair) {
            if ($pair[0] == $name) {
                ...
                if (strpbrk($name . $value, "\r\n") !== false) {
                    if ($this->exceptions) {
                        throw new \PHPMailer\PHPMailer\Exception($this->lang('invalid_header'));
                    }
                    return false;
                }
