...
class Browser extends \DeviceDetector\Parser\Client\AbstractClientParser
{
    ...
    {
        if (\in_array($browserLabel, self::$availableBrowsers, \true)) {
            $browserLabel = (string) \array_search($browserLabel, self::$availableBrowsers, \true);
        }
        foreach (self::$browserFamilies as $browserFamily => $browserLabels) {
            if (\in_array($browserLabel, $browserLabels, \true)) {
    ...
    {
        return \in_array($browser, self::$mobileOnlyBrowsers) || \in_array($browser, self::$availableBrowsers, \true) && \in_array(\array_search($browser, self::$availableBrowsers, \true), self::$mobileOnlyBrowsers, \true);
    }
    /**
    ...
    public function parse() : ?array
    {
        ...
        if (!empty($browserFromClientHints['name']) && !empty($browserFromClientHints['version'])) {
            ...
            if (('Chromium' === $name || 'Chrome Webview' === $name) && !empty($browserFromUserAgent['name']) && !\in_array($browserFromUserAgent['short_name'], ['CR', 'CV', 'AN', 'CM'])) {
    ...
    protected function parseBrowserFromClientHints() : array
    {
        ...
        if ($this->clientHints instanceof ClientHints && $this->clientHints->getBrandList()) {
            foreach ($this->clientHints->getBrandList() as $brand => $brandVersion) {
                ...
                if (!\in_array($name, ['', 'Chromium', 'Microsoft Edge'], \true)) {
                    break;
                }
            }
