...
class OperatingSystem extends \DeviceDetector\Parser\AbstractParser
{
     ...
     * @var array
     */
    protected static $desktopOsArray = ['AmigaOS', 'IBM', 'GNU/Linux', 'Mac', 'Unix', 'Windows', 'BeOS', 'Chrome OS', 'OpenVMS'];
    /**
     * Fire OS version mapping
     *
     * @var array
     */
    private $fireOsVersionMapping = ['14' => '14', '13' => '14', '12' => '14', '11' => '8', '10' => '8', '9' => '7', '7' => '6', '5' => '5', '4.4.3' => '4.5.1', '4.4.2' => '4', '4.2.2' => '3', '4.0.3' => '3', '4.0.2' => '3', '4' => '2', '2' => '1'];
    /**
     * Lineage OS version mapping
     *
     * @var array
     */
    private $lineageOsVersionMapping = ['16' => '23', '15' => '22', '14' => '21', '13' => '20.0', '12.1' => '19.1', '12' => '19.0', '11' => '18.0', '10' => '17.0', '9' => '16.0', '8.1.0' => '15.1', '8.0.0' => '15.0', '7.1.2' => '14.1', '7.1.1' => '14.1', '7.0' => '14.0', '6.0.1' => '13.0', '6.0' => '13.0', '5.1.1' => '12.1', '5.0.2' => '12.0', '5.0' => '12.0', '4.4.4' => '11.0', '4.3' => '10.2', '4.2.2' => '10.1', '4.0.4' => '9.1.0'];
    /**
    ...
    public function parse() : ?array
    {
        ...
        if ('' !== $name && !\in_array($name, self::$operatingSystems)) {
            ...
            throw new \Exception(\sprintf("The operating system '%s' should be listed in operatingSystems array. Tried to parse user agent: %s", $name, $this->userAgent));
            // @codeCoverageIgnore
        }
        $return = ['name' => $name, 'short_name' => $short, 'version' => $version, 'platform' => $platform, 'family' => $family];
        if (\in_array($return['name'], self::$operatingSystems, \true)) {
            $return['short_name'] = \array_search($return['name'], self::$operatingSystems, \true);
        }
        return $return;
    }
    /**
    ...
    {
        if (\in_array($osLabel, self::$operatingSystems, \true)) {
            $osLabel = (string) \array_search($osLabel, self::$operatingSystems, \true);
        }
        foreach (self::$osFamilies as $family => $labels) {
            if (\in_array($osLabel, $labels, \true)) {
                return $family;
            }
        }
        return null;
    ...
    public static function isDesktopOs(string $osName) : bool
    {
        ...
        return \in_array($osFamily, self::$desktopOsArray, \true);
    }
    /**
