...
<form method="post">
	...
	<table class="matomo-tracking-form widefat">
		...
		$form->show_textarea( 'noscript_code', __( 'Noscript code', 'matomo' ), 2, 'This is a preview of your &lt;noscript&gt; code which is part of your tracking code.', $is_not_tracking, 'matomo-track-option matomo-track-option-default  matomo-track-option-manually', true, '', ( $settings->get_global_option( 'track_mode' ) !== 'manually' ), false );
		...
		echo '<span class="dashicons dashicons-editor-help" style="cursor: pointer;" onclick="jQuery(\'#add_post_annotations-desc\').toggleClass(\'hidden\');"></span> <p class="description hidden" id="add_post_annotations-desc">' . sprintf( __( 'See %1$sMatomo documentation%2$s.', 'matomo' ), '<a href="https://matomo.org/docs/annotations/" target="_BLANK">', '</a>' ) . '</p></td></tr>';

		$form->show_input( 'add_download_extensions', __( 'Add new file types for download tracking', 'matomo' ), __( 'Add file extensions for download tracking, divided by a vertical bar (&#124;).', 'matomo' ) . ' ' . sprintf( __( 'See %1$sMatomo documentation%2$s.', 'matomo' ), '<a href="https://developer.matomo.org/guides/tracking-javascript-guide#tracking-file-downloads" target="_BLANK">', '</a>' ), $is_not_generated_tracking, $full_generated_tracking_group );

		$form->show_checkbox( 'disable_cookies', __( 'Disable cookies', 'matomo' ), __( 'Disable all tracking cookies for a visitor.', 'matomo' ), $is_not_generated_tracking, $full_generated_tracking_group );

		$form->show_checkbox( 'limit_cookies', __( 'Limit cookie lifetime', 'matomo' ), __( 'You can limit the cookie lifetime to avoid tracking your users over a longer period as necessary.', 'matomo' ), $is_not_generated_tracking, $full_generated_tracking_group, true, 'jQuery(\'tr.matomo-cookielifetime-option\').toggleClass(\'matomo-hidden\');' );

		$form->show_input( 'limit_cookies_visitor', __( 'Visitor timeout (seconds)', 'matomo' ), false, $is_not_generated_tracking || ! $settings->get_global_option( 'limit_cookies' ), $full_generated_tracking_group . ' matomo-cookielifetime-option' . ( $settings->get_global_option( 'limit_cookies' ) ? '' : ' matomo-hidden' ) );

		$form->show_input( 'limit_cookies_session', __( 'Session timeout (seconds)', 'matomo' ), false, $is_not_generated_tracking || ! $settings->get_global_option( 'limit_cookies' ), $full_generated_tracking_group . ' matomo-cookielifetime-option' . ( $settings->get_global_option( 'limit_cookies' ) ? '' : ' matomo-hidden' ) );

		$form->show_input( 'limit_cookies_referral', __( 'Referral timeout (seconds)', 'matomo' ), false, $is_not_generated_tracking || ! $settings->get_global_option( 'limit_cookies' ), $full_generated_tracking_group . ' matomo-cookielifetime-option' . ( $settings->get_global_option( 'limit_cookies' ) ? '' : ' matomo-hidden' ) );

		$form->show_checkbox( 'track_admin', __( 'Track admin pages', 'matomo' ), __( 'Enable to track users on admin pages (remember to configure the tracking filter appropriately).', 'matomo' ), $is_not_tracking, $full_generated_tracking_group . ' matomo-track-option-manually matomo-track-option-tagmanager' );

		$form->show_checkbox( 'track_across', __( 'Track subdomains in the same website', 'matomo' ), __( 'Adds *.-prefix to cookie domain.', 'matomo' ) . ' ' . sprintf( __( 'See %1$sMatomo documentation%2$s.', 'matomo' ), '<a href="https://developer.matomo.org/guides/tracking-javascript-guide#tracking-subdomains-in-the-same-website" target="_BLANK">', '</a>' ), $is_not_generated_tracking, $full_generated_tracking_group );

		$form->show_checkbox( 'track_across_alias', __( 'Do not count subdomains as outlink', 'matomo' ), __( 'Adds *.-prefix to tracked domain.', 'matomo' ) . ' ' . sprintf( __( 'See %1$sMatomo documentation%2$s.', 'matomo' ), '<a href="https://developer.matomo.org/guides/tracking-javascript-guide#outlink-tracking-exclusions" target="_BLANK">', '</a>' ), $is_not_generated_tracking, $full_generated_tracking_group );
		...
		$form->show_input( 'set_download_classes', __( 'Set classes to be treated as downloads', 'matomo' ), __( 'Set classes to be treated as downloads (in addition to piwik_download), divided by a vertical bar (&#124;). Leave blank to keep Matomo\'s default settings.', 'matomo' ) . ' ' . sprintf( __( 'See %1$sMatomo JavaScript Tracking Client reference%2$s.', 'matomo' ), '<a href="https://developer.matomo.org/api-reference/tracking-javascript" target="_BLANK">', '</a>' ), $is_not_tracking, $full_generated_tracking_group );

		$form->show_input( 'set_link_classes', __( 'Set classes to be treated as outlinks', 'matomo' ), __( 'Set classes to be treated as outlinks (in addition to piwik_link), divided by a vertical bar (&#124;). Leave blank to keep Matomo\'s default settings.', 'matomo' ) . ' ' . sprintf( __( 'See %1$sMatomo JavaScript Tracking Client reference%2$s.', 'matomo' ), '<a href="https://developer.matomo.org/api-reference/tracking-javascript" target="_BLANK">', '</a>' ), $is_not_tracking, $full_generated_tracking_group );
